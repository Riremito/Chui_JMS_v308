...
public class PacketHelper {
    ...
    public static void addStartedQuestInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        int v102 = 0;
        mplew.writeShort(v102);
        for (int i = 0; i < v102; i++) {
...
/*                if (((Skill) skill.getKey()).getId() == 40020002 || ((Skill) skill.getKey()).getId() == 80000004) {
    ...
    public static void addMoneyInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        mplew.writeInt/*Long*/((int)chr.getMeso());
    }
    ...
    public static void addInventoryInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        for (Item item : equipped) {
            if ((item.getPosition() <= -1500) && (item.getPosition() > -1600)) { // 花狐裝備
                addItemPosition(mplew, item, false, false);
                addItemInfo(mplew, item, chr);
            }
        }
...
//        mplew.writeInt(0);
//        mplew.writeInt(0);
        mplew.writeInt(chr.getExtendedSlots().isEmpty() ? -1 : chr.getExtendedSlots().size());
    ...
    public static void addCharStats(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        mplew.write(4/*6*/); // 日版是5 台版是6 用台版的mask凱撒1~4轉會終止遊戲
        mplew.write(4/*7*/); // 日版是5 台版是7 用台版的mask凱撒1~4轉會終止遊戲
        mplew.writeInt(0);
    ...
    public static void addItemInfo(final MaplePacketLittleEndianWriter mplew, final Item item, final MapleCharacter chr) {
        ...
        if (item.getPet() != null) { // Pet
            ...
            if (item.getType() == 1) {
                ...
                mplew.write(equip.getState(true) > 0 && equip.getState(true) < 17 ? equip.getState(false) | 0x20 : equip.getState(false)/*equip.getState()*/); // 17 = rare, 18 = epic, 19 = unique, 20 = legendary, potential flags.
                mplew.write(equip.getEnhance());
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(1, false));
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(2, false));
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(3, false));
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(4, false));
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(5, false));
                mplew.writeShort(equip.getState(false) > 0 && equip.getState(false) < 17 ? 0 : equip.getPotential(6, false));
...
/*                mplew.writeShort(0);
                ...
                mplew.writeInt/*Short*/(equip.getSocket(1) % 10000); // > 0 = mounted, 0 = empty, -1 = none.
                mplew.writeInt/*Short*/(equip.getSocket(2) % 10000);
                mplew.writeInt/*Short*/(equip.getSocket(3) % 10000);
...
/*                final Equip equip = Equip.calculateEquipStats((Equip) item);
                ...
                if (item.getItemId() / 10000 == 287) {
                    mplew.writeInt(100);
                    mplew.writeShort(100);
                    mplew.writeShort(100);
                    mplew.writeShort(100);
                    mplew.writeShort(100);
                    mplew.writeLong(System.currentTimeMillis() + 60);
                    mplew.writeLong(System.currentTimeMillis() + 60);
                    mplew.writeLong(System.currentTimeMillis() + 60);
                    mplew.writeLong(System.currentTimeMillis() + 60);
                    mplew.writeInt(100);
...
//        mplew.writeLong(/*equip.getInventoryId()*/-1); //some tracking ID
        ...
        if (equip.getItemId() / 10000 == 171) { // 祕法符文
            mplew.writeShort(500/*equip.getARC()*/);
            mplew.writeInt(3000/*equip.getARCExp()*/);
            mplew.writeShort(100/*equip.getARCLevel()*/);
        }
        //突破傷害上限
    ...
    public static void addInteraction(MaplePacketLittleEndianWriter mplew, IMaplePlayerShop shop) {
        ...
        mplew.write(shop.getShopType() != 75 ? shop.getItemId() % 100 : 0/*shop.getItemId() % 10*/);
        mplew.write(shop.getShopType() != 75 ? shop.getSize() : 0/*shop.getSize()*/);
...
//        long mask = 0xFF_FF_FF_FF_FF_FF_FF_FFL; //FF FF FF FF FF FF DF FF v148+
        ...
        mplew.writeInt(chr.getId());
        mplew.writeLong(0); // 小鋼珠
        mplew.writeInt(0);
        mplew.writeLong(0);
//        mplew.writeInt(chr.getCSPoints(2)); // 楓葉點數

        if ((mask & 8 | mask & 0x2000000) != 0) { // EXP藥水詳細訊息
            int v20 = 0;
            mplew.writeInt(v20);
            for (int i = 0; i > v20; i++) {
                mplew.writeInt(0);
        ...
        addMonsterBookInfo(mplew, chr); // 0x20000 0x10000 0x200
        ...
        for (int i = 0; i < 10; i++) {
            mplew.write(0); // 如果 > 0 要加additeminfo
        }
...
//        mplew.write(ServerConstants.SHOP_DISCOUNT);//打折
        ...
        mplew.writeInt(0/*item.getCategory()*/);
        mplew.writeInt(0/*item.getExpiration()*/);
        mplew.writeInt(0/*ItemConstants.類型.裝備(item.getItemId()) && item.hasPotential() ? 1 : 0*/);
/*        mplew.writeInt(0);//組隊積分類型
        mplew.writeInt(0);//組隊積分
        mplew.writeInt(0);//明星M積分數量
        mplew.write(0); // boolean
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0); // 道具剩餘數量
        mplew.writeInt(item.getMinLevel()); //購買等級限制
        mplew.writeShort(0);
        mplew.writeShort(0); // > 0 的話道具不會顯示
        mplew.write(0); // boolean
        mplew.write(0); // > 0 的話道具不會顯示*/
        mplew.writeLong(getTime(-2));
        mplew.writeLong(getTime(-1));
        mplew.writeInt(0); // > 0 的話道具不會顯示
        mplew.write/*Int*/(0);
//        mplew.writeMapleAsciiString("333333");
        mplew.writeInt(0);
...
//        mplew.writeMapleAsciiString("2079010100");
        ...
        mplew.write(i == null ? 0 : 1); // 重新購買道具的信息
        if (i != null) {
            addItemInfo(mplew, i);
        }
        if (shop.getRanks().size() > 0) {
...
//        }
        ...
        // 預設Buff
        statups.put(MapleBuffStat.DEFAULTBUFF0, null);
        // 預設Buff
        ...
        // 預設Buff - 内容猜填
        statups.put(MapleBuffStat.DEFAULTBUFF6, new Object[]{(short) 0, 0});
        // 預設Buff - 内容猜填
        statups.put(MapleBuffStat.DEFAULTBUFF7, new Object[]{(short) 0, 0});
        // 預設Buff - 内容猜填
        statups.put(MapleBuffStat.DEFAULTBUFF8, null);
        // 預設Buff
...
//        });
        ...
        mplew.write(HexTool.getByteArrayFromHexString("00 00 00 FE 00 00 00 00 00 00 00 00 00 00 00 00"
