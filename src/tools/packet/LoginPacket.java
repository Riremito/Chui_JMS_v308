...
public class LoginPacket {
    ...
    public static byte[] getHello(byte[] sendIv, byte[] recvIv) {
        ...
        mplew.writeShort(14/*15*/); // 數據長度(不包含這裡)
...
//        mplew.write(0);
    ...
    public static final byte[] showMapleStory() {
        ...
        mplew.write/*Int*/(1); // 台服是0 日服是1 大概用來顯示載入畫面的文字用
    ...
    public static byte[] getLoginBackground() {
        ...
        String[] bg = {/*"MapLogin", "MapLogin0", "MapLogin1", "MapLogin2"*/"MapLogin"};
...
//        mplew.write(1);
    ...
    public static final void getAuthSuccess(MaplePacketLittleEndianWriter mplew, MapleClient client, boolean second) {
        ...
        if (!second) {
            ...
            mplew.write(0);
            mplew.write(0);
            mplew.write(1); // 0 = 未設置第二組 1 = 已設置第二組
            mplew.write(1);
        }
...
//        mplew.write(0);
    ...
    public static byte[] getJobOpen() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.JOB_OPEN.getValue());
        mplew.writeInt(2);

        return mplew.getPacket();
    }

    public static byte[] getCharList(String secondpw, List<Integer> charPos, List<MapleCharacter> chars, int charslots, boolean reboot) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
