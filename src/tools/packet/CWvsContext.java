...
//    }
    ...
    public static class BuddylistPacket {
        ...
        public static byte[] updateBuddylist(Collection<BuddylistEntry> buddylist) {
            return updateBuddylist(buddylist, 0x17);
        }
        ...
        public static byte[] updateBuddylist(Collection<BuddylistEntry> buddylist, int deleted) {
            ...
            for (BuddylistEntry buddy : buddylist) {
                ...
                mplew.write(buddy.isVisible() ? 0 : 1); //0普通好友不在線 2普通好友在線 4開啟帳號轉換,5離線帳號好友,7帳號好友在線
                ...
                mplew.write(HexTool.getByteArrayFromHexString("00 AE 1A 0B 64 FC 34 11 10 0D AB 0F 68"));
...
/*            for (int x = 0; x < buddylist.size(); x++) {
        ...
        public static byte[] requestBuddylistAdd(int cidFrom, String nameFrom, int levelFrom, int jobFrom) {
            ...
            mplew.write(1);
            mplew.writeInt(0);
            mplew.writeAsciiString("群組未指定", 18);
            mplew.writeInt(0);
            mplew.writeAsciiString(nameFrom, 13);
            for (int i = 0; i < 65; i++) {
                mplew.writeInt(0);
...
//            mplew.writeMapleAsciiString(party.getName());
        ...
        public static byte[] partyRequestInvite(MapleCharacter from) { // 隊伍邀請回覆
            ...
            mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
            mplew.write(0x07); // Version.176 [完成]
...
/*            for (MaplePartyCharacter partychar : partymembers) {
        ...
        public static byte[] partyPortal(int townId, int targetId, int skillId, Point position, boolean animation) {
            ...
            mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
            mplew.write(0x51); // Version.176 [完成]
        ...
        public static byte[] getPartyListing(PartySearchType pst) {
            ...
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
            mplew.write(0x61); // Version.176 [推測]
        ...
        public static byte[] partyListingAdded(PartySearch ps) {
            ...
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
            mplew.write(0x71); // Version.176 [推測]
    ...
    public static class GuildPacket {
        ...
        public static byte[] showGuildInfo(MapleCharacter c) {
            ...
            mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
            mplew.write(0x20); // Version.176 [完成]
...
/*            mplew.writeInt(25); // 大小
        ...
        public static byte[] guildMemberLevelJobUpdate(MapleGuildCharacter mgc) {
            ...
            mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
            mplew.write(0x5F/*0x5C*/); // Version.176 [完成]
            mplew.writeInt(mgc.getGuildId());
            mplew.writeInt(mgc.getId());
            mplew.writeInt(mgc.getLevel());
            mplew.writeInt(mgc.getJobId());
        ...
        public static byte[] guildMemberOnline(int gid, int cid, boolean bOnline) {
            ...
            mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
            mplew.write(0x60/*0x5D*/); // Version.176 [推測]
            mplew.writeInt(gid);
            mplew.writeInt(cid);
...
/*            mplew.write(0);
    ...
    public static class InfoPacket {
        ...
        public static byte[] monsterCombo(int Combo, int moid) {
            if (ServerConfig.LOG_PACKETS) {
                System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
            }
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
/*        public static byte[] giveMount(int buffid, int skillid, Map<MapleBuffStat, Integer> statups) {
        ...
        public static byte[] showMonsterRiding(int cid, List<Pair<MapleBuffStat, Integer>> statups, int buffid, int skillId) {
            if (ServerConfig.LOG_PACKETS) {
                System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
            }
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect, MapleCharacter chr) {
            ...
            for (Map.Entry<MapleBuffStat, Integer> stat : statups.entrySet()) {
                if (!stat.getKey().canStack() && stat.getKey() != MapleBuffStat.DEFAULTBUFF1) {
...
//            IsBaseBuff(mplew, statups, effect, chr, buffid, bufflength);
            ...
            // 這裡是堆疊Buff SecondaryStat::DecodeIndieTempStat
            for (Map.Entry<MapleBuffStat, Integer> stat : statups.entrySet()) {                
                if (stat.getKey().canStack() && !GameConstants.isSpecialStackBuff(stat.getKey())) {
...
//                mplew.writeInt(0);
            ...
            if (statups.containsKey(MapleBuffStat.IDK_UNK_267)) {
                mplew.writeInt(0);
            }
...
//                    System.out.println("堆疊Buff::" + stat.name()); // 查看排列順序用
                    List<MapleBuffStatValueHolder> buffs = chr.getStackBuffStatValueHolder(stat);
                    if (buffs == null) {
                        continue;
                    }
...
//                        mplew.writeInt(((int)currentTime) - ctime);
        ...
        public static byte[] get黑暗之眼(int level) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
            PacketHelper.writeSingleMask(mplew, MapleBuffStat.PRESSURE_VOID);
            mplew.writeShort(level);
...
//        }
        ...
        public static byte[] getDoubleDefense(int buffid, List<Pair<MapleBuffStat, Integer>> statups, MapleCharacter chr, int x) {//双重防御
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static class Enchant {
        ...
        public static byte[] getEnchantList(MapleClient c, Equip item, ArrayList<EchantScroll> scrolls, int count_rate, boolean feverTime) {
            ...
            for (EchantScroll scroll : scrolls) {
                ...
                mplew.writeInt(scroll.getViewType() == 4 ? 1 : (scroll.getViewType() == 5 ? 2 : 0));
                mplew.writeInt(scroll.getViewType() >= 4 ? 1 : 0);
        ...
        public static byte[] getStarForcePreview(final Map<EchantEquipStat, Integer> stats, long meso, int successprop, int destroytype, boolean canfall, boolean chancetime) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] getStarForceMiniGame(int code) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//                mplew.write(HexTool.getByteArrayFromHexString("00 C4 2F 63"));
        ...
        public static byte[] getEnchantResult(Equip item, int result, int scrollnumber, String ScrollName) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            ...
            mplew.writeMapleAsciiString(ScrollName);
