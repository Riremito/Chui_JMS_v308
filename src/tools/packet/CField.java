...
import static tools.packet.PacketHelper.addCompletedQuestInfo;
import static tools.packet.PacketHelper.addStartedQuestInfo;
...
*/
    ...
    public static byte[] SetField2(MapleCharacter chr) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.WARP_TO_MAP.getValue());
        ...
        int v26 = 0;
        mplew.writeShort(v26); // 這應該也是任務
        for (int i = 0; i < v26; i++) {
            mplew.writeShort(0);
            mplew.writeLong(0);
        }
        
        // CharacterData::Deocde2結束

        mplew.writeLong(PacketHelper.getTime(System.currentTimeMillis()));
        mplew.writeInt(100); // paramFieldInit.nMobStatAdjustRate
        mplew.writeBoolean(false);
        mplew.write(0);
        mplew.write(GameConstants.isSeparatedSp(chr.getJob()) ? 1 : 0);
        mplew.write(0);
...
//        mplew.writeZeroBytes(8);
    ...
    public static byte[] showEnvironment(int mode, String env, int[] values) {
        ...
        switch (mode) {
            ...
            case 5: // BOSS血條
                ...
                mplew.writeInt(values[1]); // HP
                mplew.writeInt(values[2]); // MAXHP
            ...
            case 22:
                ...
                break;
            case 25:
                mplew.writeBoolean(values[0]); // isBinary
                mplew.writeBoolean(values[1]); // isLoop
                mplew.writeBoolean(values[2]); // isPostRender
                mplew.writeInt(values[3]);
                mplew.writeMapleAsciiString("Map/Effect3.img/BossLucid/Lucid/lusi");
                mplew.writeMapleAsciiString("animation");
                mplew.writeBoolean(values[4]); // use key
                if (values[4] != 0) {
                    mplew.writeMapleAsciiString(""); // the key to stop animation?
                }
                break;
        }
    ...
    public static byte[] environmentChange(String env, int mode) {
        return showEnvironment(mode, env, new int[]{0});
    }

    public static byte[] trembleEffect(int type, int delay) {
        return showEnvironment(1, null, new int[]{type, delay, 30});
    }
...
//            mplew.writeLong(PacketHelper.getTime(-1));
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        mplew.writeInt(5); // // 性向 0=領導力 1=洞察 2=意志 3=手藝 4=感性 5=魅力
        mplew.writeInt(6); // 性向等級 100等是6
        mplew.write(1); // 性向暱稱開關 角色名稱左邊出現的 0=關 1=開
        mplew.write(1); // 性向效果開關 頭上會出現的 0=關 1=開
        ...
        if ((chr.getBuffedValue(MapleBuffStat.SOARING) != null) && (buffSrc > 0)) {//妮娜的魔法陣 1C 7B 1D 00 //5C 58 8A 00
        ...
        mplew.write(0/*chr.getHaku() != null && MapleJob.is陰陽師(chr.getJob())*/);
...
/*        int v65 = 0;
        ...
        int unk_mask = 0;
        mplew.write(unk_mask/*chr.getStat().Berserk ? 1 : 0*/);
        if ((unk_mask & 1) != 0) {
        }
        if ((unk_mask & 2) != 0) {
        }
        ...
        mplew.writeInt(0/*chr.getMount().getItemId()*/);//騎寵id
...
/*        mplew.writeInt(0);
        ...
        for (int i = 0; i < 5; i++) { // 不發送這5個-1就會看到人物頭上稱號一大堆
            mplew.write(-1);
        }
        int v84 = 0;
        mplew.writeInt(v84);
    ...
    public static byte[] spawnAndroid(MapleCharacter cid, MapleAndroid android) {
        ...
        for (short i = -1200; i > -1207/*-1208*/; i--) {
    ...
    public static byte[] closeRangeAttack(MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo, boolean hasMoonBuff) {
        return addAttackInfo(SendPacketOpcode.CLOSE_RANGE_ATTACK, chr, skilllevel, itemId, attackInfo, hasMoonBuff);
    }

    public static byte[] rangedAttack(MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo) {
        return addAttackInfo(SendPacketOpcode.RANGED_ATTACK, chr, skilllevel, itemId, attackInfo, false);
    }

    public static byte[] magicAttack(MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo) {
        return addAttackInfo(SendPacketOpcode.MAGIC_ATTACK, chr, skilllevel, itemId, attackInfo, false);
    }

    public static byte[] passiveAttack(MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo, boolean hasMoonBuff) {
        return addAttackInfo(SendPacketOpcode.PASSIVE_ATTACK, chr, skilllevel, itemId, attackInfo, hasMoonBuff);
    }

    public static byte[] addAttackInfo(SendPacketOpcode op, MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo, boolean hasMoonBuff) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static void addAttackBody(MaplePacketLittleEndianWriter mplew, SendPacketOpcode op, MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo, boolean hasMoonBuff) {
        mplew.writeInt(chr.getId());
//        mplew.write(op == SendPacketOpcode.RANGED_ATTACK);
...
/*        if (attackInfo.skill == 80001850 || attackInfo.skill == 42001000 || attackInfo.skill > 42001004 && attackInfo.skill <= 42001006) {
            mplew.write(0);
        }
        if (hasMoonBuff) {
            mplew.write(0);
        ...
        if (op == SendPacketOpcode.RANGED_ATTACK) {
            mplew.write(8);
        } else {
            mplew.write(GameConstants.Attacktype(attackInfo.skill));
        }*/
        if (attackInfo.skill == 3211006 || attackInfo.skill == 40021185 && attackInfo.skill == 42001006) {
            mplew.write(0);
        }
        mplew.write(attackInfo.unk);//always 0?
/*        mplew.writeInt(0);
        mplew.writeInt(0);
        if ((attackInfo.unk & 2) != 0) {
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if ((attackInfo.unk & 8) != 0) {
            mplew.write(0);
        }*/
...
//                        }
        ...
        if (op == SendPacketOpcode.RANGED_ATTACK) {
            if (attackInfo.skillposition != null) {
...
/*        switch (attackInfo.skill) {
        ...
        if (attackInfo.skill == 2121001 || attackInfo.skill == 2221001 || attackInfo.skill == 2321001) {
            mplew.writeInt(0);
        } else if (attackInfo.skill == 65121052) {
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (attackInfo.skill == 42100007) {
            mplew.writeShort(0);
            int size = 0;
            mplew.write(size);
            for (int i = 0; i < size; i++) {
...
//            mplew.writeInt(cid);
    ...
    public static byte[] instantMapWarp(final int portal) {
        return instantMapWarp(0, portal, 0, new Point(0, 0));
    }

    public static byte[] instantMapWarp(final int opcode, final int portal, final int charId, final Point pos) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
/*        byte unk = 0;
        ...
        for (ObtacleAtom bomb : bombs) {
            ...
            mplew.writeInt(Randomizer.rand(1, 8)/*bomb.getType()*/);            
...
//        mplew.writeInt(1);
    ...
    public static byte[] spawnMist(MapleMist mist) {
        ...
        mplew.write(0);
        mplew.writeInt(0);
        Skill skil = mist.getSourceSkill();
        if (skil == null) {
            mplew.writeInt(0);
        } else {
            switch (skil.getId()) {
...
//            mplew.writeInt(0); // 178+
        ...
        public static byte[] getNPCTalk_IDA(String talk, int msgType, int type, int npc, int type2, int type3, int type4, int type5, int type6, int type7) {
            if (ServerConfig.LOG_PACKETS) {
                System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
            }
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//            mplew.write(0); // 175+
        ...
        public static byte[] getNPCTalkStyle(int npc, String talk, int[] args, boolean second) {
            ...
            mplew.writeInt(npc);
//            mplew.write(0); // Boolean
            mplew.write(NPCTalk.AVATAR.getType());
            mplew.write(0);
//            mplew.write(0);
            mplew.write(0);
        ...
        public static byte[] getNPCTalkNum(int npc, String talk, int def, int min, int max) {
            ...
            mplew.writeInt(npc);
//            mplew.write(0); // Boolean
            mplew.write(NPCTalk.NUMBER.getType());
            mplew.write(0);
//            mplew.write(0);
            mplew.write(0);
            mplew.writeMapleAsciiString(talk);
        ...
        public static byte[] getNPCTalkText(int npc, String talk) {
            ...
            mplew.writeInt(npc);
//            mplew.write(0); // Boolean
            mplew.write(NPCTalk.TEXT.getType()); // 3 regular 6 quiz
            mplew.write(0);
//            mplew.write(0);
            mplew.write(0);
            mplew.writeMapleAsciiString(talk);
...
//            mplew.write(0);
        ...
        public static byte[] getAngelicBusterAvatarSelect(int npc) {
            ...
            mplew.writeInt(npc);
            mplew.write(0); // Boolean
            mplew.write(NPCTalk.ANGELIC_AVATAR.getType());
//            mplew.write(0);
            mplew.write(0);
            mplew.write(0);
...
//            mplew.write(0); // Boolean
        ...
        public static byte[] getAndroidTalkStyle(int npc, String talk, int... args) {
            ...
            mplew.writeInt(npc);
//            mplew.write(0); // Boolean
            mplew.write(NPCTalk.ANDROID.getType());
            mplew.write(0);
//            mplew.write(0);
            mplew.writeMapleAsciiString(talk);
            mplew.write(args.length);
...
/*            mplew.write(0);
        ...
        public static byte[] summonAttack(int cid, int summonSkillId, byte animation, byte numAttackedAndDamage, List<AttackPair> allDamage, int level, boolean darkFlare) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static class UIPacket {
        ...
        public static byte[] getDirectionInfo(int mod, String data, int[] value) {
            ...
            mplew.write(mod == 9 ? 7 : mod == 10 ? 8 : mod);
            switch (mod) {
                case 0:
...
//            mplew.write(0);
    ...
    public static class EffectPacket {
        ...
        public static byte[] showMonsterBookEffect() {
            return showEffect(true, null, SpecialEffectType.MONSTER_BOOK, null, null, null, null);
        }
        ...
        public static byte[] showBuffEffect(boolean self, MapleCharacter chr, int skillid, SpecialEffectType effect, int playerLevel, int skillLevel, byte direction) {
            ...
            mplew.write(skillLevel);
            mplew.write(direction); //角色等級 好像有些需要寫角色等級
            if (skillid == 4331006) {
                mplew.writeInt(0);
            }
    ...
    public static byte[] 性向稱號獲得(int cid) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.性向稱號獲得.getValue());
        mplew.writeInt(cid);
        mplew.writeInt(5); // // 性向 0=領導力 1=洞察 2=意志 3=手藝 4=感性 5=魅力
        mplew.writeInt(6); // 性向等級 100等是6
