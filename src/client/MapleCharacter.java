...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    private long lastCombo, lastfametime, keydown_skill, nextConsume, pqStartTime, lastDragonBloodTime,
            ...
            lastHPTime, lastMPTime, lastRecoveryTimeEM, lastFamiliarEffectTime, lastDOTTime, exp, meso, lastExceedTime;
    ...
    private short level, job, mulung_energy, combo, force, availableCP, fatigue, totalCP, hpApUsed, scrolledPosition,
            kaiserCombo, xenonSurplus, soulcount = 0, exceed, exceedAttack = 0;
    private int accountid, id, hair, face, secondHair, secondFace, faceMarking, ears, tail, mapid, fame, pvpExp, pvpPoints,
            guildid = 0, fallcounter, maplepoints, mileage, beanfunGash, chair, itemEffect,
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        ret.job = (short) (type == JobType.幻獸師 ? MapleJob.幻獸師4轉.getId() : type.id);
        ret.meso = 100000;
        ret.level = 1;
        ret.remainingAp = 0;
        ret.fame = 0;
        ...
        if (guidebook > 0) {
            ret.getInventory(MapleInventoryType.ETC).addItem(new Item(guidebook, (byte) 0, (short) 1, (byte) 0));
        }

        if (MapleJob.is幻影俠盜(ret.getJob())) {
            ret.getStat().maxhp = 850;
            ret.getStat().hp = 140;
            ret.getStat().maxmp = 805;
            ret.getStat().mp = 38;
        }
        ...
        if (MapleJob.is神之子(ret.getJob())) {
            ...
            ret.setQuestAdd(MapleQuest.getInstance(40900), (byte) 2, "");
            ret.setQuestAdd(MapleQuest.getInstance(40901), (byte) 2, "");
            ret.setQuestAdd(MapleQuest.getInstance(40902), (byte) 2, "");
            ret.setQuestAdd(MapleQuest.getInstance(40903), (byte) 2, "");
            ret.setQuestAdd(MapleQuest.getInstance(40904), (byte) 2, "");
            ret.setQuestAdd(MapleQuest.getInstance(40905), (byte) 2, "");
        }
    ...
    public static MapleCharacter ReconstructChr(final CharacterTransfer ct, final MapleClient client, final boolean isChannel) {
        ...
        ret.beanfunGash = ct.beanfunGash;
        ret.mileage = ct.mileage;
    ...
    public static void saveNewCharToDB(final MapleCharacter chr, final JobType type, short db, int keymapType) {
        ...
        try {
            ...
            ps = con.prepareStatement("INSERT INTO characters (level, str, dex, luk, `int`, hp, mp, maxhp, maxmp, sp, hsp, ap, skincolor, gender, job, hair, face, faceMarking, ears, tail, weaponPoint, map, meso, party, buddyCapacity, pets, subcategory, friendshippoints, chatcolour, gm, accountid, name, world, position)"
    ...
    public void saveToDB(boolean dc, boolean fromcs) {
        ...
        try {
            ...
            ps = con.prepareStatement("UPDATE accounts SET `BeanfunGash` = ?, `Mileage` = ?, `mPoints` = ?, `lastWorld` = ? WHERE id = ?");
    ...
    public final void updateQuest(final MapleQuestStatus quest, final boolean update) {
        ...
        if (!(quest.isCustom())) {
            ...
            if (quest.getStatus() == 1 && !update) {
                client.announce(CField.updateQuestInfo(this, quest.getQuest().getId(), quest.getNpc(), (byte) 11));//was10
            }
        }
    }
    ...
    public MapleBuffStatValueHolder getBuffStatValueHolder(MapleBuffStat effect) {
    ...
    public Integer getBuffedValue(MapleBuffStat effect) {
        if (effect.canStack() && !GameConstants.isSpecialStackBuff(effect)) {
            Integer i = null;
            if (!stack_effects.containsKey(effect)) {
                return i;
            }
            for (MapleBuffStatValueHolder mbsvh : stack_effects.get(effect)) {
                if (i == null) {
                    i = 0;
                }
                i += mbsvh.value;
    ...
    public void doDragonBlood() {
        ...
        if (stats.getHp() - bloodEffect.getX() <= 1) {
            ...
            client.announce(EffectPacket.showBuffEffect(true, this, bloodEffect.getSourceId(), SpecialEffectType.ITEM_OPERATION, getLevel(), bloodEffect.getLevel()));
            map.broadcastMessage(MapleCharacter.this, EffectPacket.showBuffEffect(false, this, bloodEffect.getSourceId(), SpecialEffectType.ITEM_OPERATION, getLevel(), bloodEffect.getLevel()), false);
        }
    }
    ...
    public void doRecoveryEM() {
        MapleStatEffect bloodEffect = getStatForBuff(MapleBuffStat.DIABOLIC_RECOVERY);
        if (bloodEffect == null) {
    ...
    public boolean canRecoverEM(long now) {
        MapleStatEffect effect = getStatForBuff(MapleBuffStat.DIABOLIC_RECOVERY);
        if (effect == null) {
            lastRecoveryTimeEM = 0;
            return false;
        }
    ...
    public void registerEffect(MapleStatEffect effect, long starttime, ScheduledFuture<?> schedule, Map<MapleBuffStat, Integer> statups, boolean silent, final int localDuration, final int cid) {
        ...
        if (isShowInfo()) {
            showMessage(27, new StringBuilder().append("註冊一般BUFF效果 - 當前BUFF總數： ").append(this.effects.size()).append(" 技能： ").append(effect.getSourceId()).toString());
            showMessage(27, new StringBuilder().append("註冊堆疊BUFF效果 - 當前BUFF總數： ").append(getStackBuffSize()).append(" 技能： ").append(effect.getSourceId()).toString());
        }
    }

    public int getStackBuffSize() {
        int size = 0;
        for (List<MapleBuffStatValueHolder> mbsvhs : stack_effects.values()) {
            size += mbsvhs.size();
        }
        return size;
    }
    ...
    public void cancelEffect(final MapleStatEffect effect, final boolean overwrite, final long startTime, Map<MapleBuffStat, Integer> statups) {
        ...
        for (Entry<MapleBuffStat, List<MapleBuffStatValueHolder>> stateffect : stack_effects.entrySet()) {
            ...
            for (MapleBuffStatValueHolder i : removes) {
                mbsvhs.remove(i);
            }
            if (mbsvhs.isEmpty()) {
...
//                        map.broadcastMessage(this, CField.getEffectSwitch(getId(), getEffectSwitch()), true);
        ...
        if (isShowInfo()) {
            ...
            dropMessage(5, new StringBuilder().append("取消堆疊BUFF效果 - 當前BUFF總數：").append(getStackBuffSize()).append(" 技能：").append(effect.getSourceId()).toString());
        }
    ...
    public List<PlayerBuffValueHolder> getAllBuffs() {
        ...
        for (Entry<MapleBuffStat, List<MapleBuffStatValueHolder>> mbsvhs : allStackBuffs) {
            for (MapleBuffStatValueHolder mbsvh : mbsvhs.getValue()) {
                ...
                if (skillid != 15001022) {
                    if (getBuffedValue(MapleBuffStat.ENERGY_CHARGE) == null) {
                        ...
                        if (energyLevel < 10000) {
                            ...
                            this.client.announce(CField.EffectPacket.showBuffEffect(true, this, skillid, SpecialEffectType.REMOTE_SKILL, getLevel(), skilllevel));
                            this.map.broadcastMessage(this, CField.EffectPacket.showBuffEffect(false, this, skillid, SpecialEffectType.REMOTE_SKILL, getLevel(), skilllevel), false);
...
//                        this.client.announce(CWvsContext.BuffPacket.giveEnergyChargeTest(energyLevel, echeff.getDuration() / 1000));
                    ...
                    if (getBuffedValue(MapleBuffStat.STORM_ELEMENTAL) == null) {
                        ...
                        this.client.announce(CWvsContext.BuffPacket.giveBuff(15001022, 30000, stat, echeff, this));
                    }
                }
            }
        }
    ...
    public final void handleOrbgain() {
        ...
        if (orbcount < ceffect.getX() + 1) {
            ...
            this.client.announce(CWvsContext.BuffPacket.giveBuff(combo.getId(), duration, stat, ceffect, this));
    ...
    public void handleTriflingWhim(int mobid, boolean adv, boolean storm) {
        ...
        if (skillid != 0) {
            ...
            if (storm) {
                client.announce(CField.gainForce(false, this, mobsid, 8, 13121054, forceinfo));
            } else {
                client.announce(CField.gainForce(false, this, mobsid, 7, skillid, forceinfo));
            }
        }
    }
    ...
    public void handleShadowBat(int mobid, int skill) {
        if (this.kaiserCombo > 3) {
            ...
            client.announce(CField.gainForce(false, this, mobsid, 15, 14000028, forceinfo)); // 到處飛的
//                client.announce(CField.gainForce(true, this, mobsid, 16, 14000029, forceinfo)); // 在身邊的
        }
...
//                System.out.println("暗影蝙蝠::[" + kaiserCombo + "]");
                this.kaiserCombo += 1;
                final MapleSummon tosummon = new MapleSummon(this, 14000027, 1, new Point(this.getTruePosition()), SummonMovementType.跟隨並隨機移動打怪);
...
//the sp isnt fixed oh hold on
    ...
    private void changeMapInternal(final MapleMap to, final Point pos, byte[] warpPacket, final MaplePortal pto) {
        ...
        if (map.getId() == nowmapid) {
            ...
            client.announce(CField.SetField2(this));
...
/*        if (deathCount < 1) {
            ...
            deathCount = 99;
            client.announce(EffectPacket.updateDeathCount(99)); //for fun
        }*/
    }
        ...
        try {
            ...
            if (!MapleJob.isBeginner(newJob) && newJob != MapleJob.隱忍.getId() && !MapleJob.is神之子(newJob) && !MapleJob.is幻獸師(newJob)) {
                ...
                if (GameConstants.isSeparatedSp(newJob)) {
                    ...
                    int changeSp = MapleJob.is龍魔導士(newJob) && MapleJob.getJobGrade(newJob) < 6 && MapleJob.getJobGrade(newJob) != 4 ? 3 : 5;
                    if (MapleJob.is末日反抗軍(job) && MapleJob.getJobGrade(newJob) != 1) {
                        changeSp = 3;
                    }
                    remainingSp[GameConstants.getSkillBookByJob(newJob)] += changeSp;
                    client.announce(InfoPacket.getSPMsg((byte) changeSp, (short) newJob));
                } else {
                ...
                if (MapleJob.getJobGrade(newJob) >= 3 && level >= 60) { //3rd job or higher. lucky for evans who get 80, 100, 120, 160 ap...
                    remainingAp += 5;
                    updateSingleStat(MapleStat.AVAILABLEAP, remainingAp);
                }
                if (MapleJob.getJobGrade(newJob) == 1 && level >= 10) { //first job
            ...
            if (maxhp >= 99999/*500000*/) {
                maxhp = 99999/*500000*/;
            }
            if (maxmp >= 99999/*500000*/) {
                maxmp = 99999/*500000*/;
            }
    ...
    public void addJobSkills() {
        ...
        if (MapleJob.isBeginner(job)) {
            ...
            addskills = new int[]{
                30020232, // 蓄能系統
                30020240, // 多樣化裝扮
                job >= 3610 ? 30021235 : 0, // 普羅梅莎突襲
                job != 3002 ? 30021236 : 0, // 多功能模式
                job != 3002 ? 30021237 : 0, // 自由飛行
                30020234, // 全能增幅 I
                jobNumber >= 1 ? 36000004 : 0, // 全能增幅II
                jobNumber >= 2 ? 36100007 : 0, // 全能增幅III
                jobNumber >= 3 ? 36110007 : 0, // 全能增幅IV
                jobNumber >= 4 ? 36120010 : 0, // 全能增幅V
            };
            removeskills = new int[]{
                30021238, // 刀舞
            };
    ...
    public void baseSkills() {
        ...
        for (Skill skill : SkillFactory.getAllSkills()) {
            skil = skill;
            if (skil == null || !skil.canBeLearnedBy(job) || skil.isInvisible() || getLevel() < skil.getReqLevel()) {
                continue;
            }
            int masterLevel = (byte) (ZZMSConfig.maxSkillLevel ? skil.getMaxLevel() : skil.getMasterLevel());
            SkillEntry skillEntry = skils.get(skil);
            int skillLevel = skillEntry == null ? 0 : skillEntry.skillLevel;
            if (getSkillLevel(skil) <= 0 && skil.getFixLevel() > 0) {
                list.put(skil, new SkillEntry((byte) skil.getFixLevel(), (byte) 0, SkillFactory.getDefaultSExpiry(skil)));
            } else if (masterLevel > 0 && (skillEntry == null || skillEntry.masterlevel < masterLevel)) {
                skillEntry = new SkillEntry((byte) skillLevel, (byte) masterLevel, SkillFactory.getDefaultSExpiry(skil));
    ...
    public void updateHSP() {
        for (int i = 0; i < 3; i++) {
            client.announce(CWvsContext.updateHyperSp(i, remainingHSp[i]));
        }
    }
    ...
    public void playerDead() {
        ...
        if (!MapleJob.isBeginner(job) && !inPVP() && !GameConstants.isAzwanMap(getMapId())) {
            ...
            if (amulet > 0) {
                ...
                client.announce(EffectPacket.useAmulet(1, (byte) amulet, (byte) 0));
            } else {
                float diepercentage;
        ...
        try {
            ...
            if (total != 0) {
                ...
                if (show) { // still show the expgain even if it's not there
                    client.announce(InfoPacket.GainEXP_Others(total, inChat, white));
                }
            }
        } catch (Exception e) {
            FileoutputUtil.outputFileError(FileoutputUtil.ScriptEx_Log, e); //all jobs throw errors :(
    ...
    public void gainExpMonster(int gain, final boolean show, final boolean white, byte pty, final byte Class_Bonus_EXP, final byte Premium_Bonus_EXP_PERCENT, MapleMonster mob) {
        ...
        int skilLevel = getSkillLevel(skil);
        if (skilLevel > 0 && MapleJob.is精靈遊俠(getJob())) {
            Skill_Bonus_EXP += (int) (gain * skil.getEffect(skilLevel).info.get(MapleStatInfo.expR) / 100.0D);
        } else {
            skil = SkillFactory.getSkill(80001040);
            skilLevel = getSkillLevel(skil);
            if (skilLevel > 0) {
                Skill_Bonus_EXP += (int) (gain * skil.getEffect(skilLevel).info.get(MapleStatInfo.expR) / 100.0D);
            }
        }
...
/*        int comboKill = getComboKill();
        ...
        //總經驗
        long total = gain + Party_Bonus_EXP + Wedding_Bonus_EXP + Equipment_Bonus_EXP + Internet_Cafe_Bonus_EXP + Skill_Bonus_EXP + Additional_Bonus_EXP + Class_Bonus_EXP + Buff_Bonus_EXP;
        ...
        if (level >= maxLevel) {
            ...
            if (exp + total >= needed || exp >= needed) {
                ...
                while (exp > needed) {
                    levelUp();
                    needed = getNeededExp();
                    if (ZZMSConfig.levelUpTimesLimit) {
                        break;
                    }
                }
                leveled = true;
        ...
        if (gain != 0) {
            ...
            if (show) { // still show the expgain even if it's not there
                ...
                // 加持獎勵經驗值
                expmasks.add(new Pair<>(ExpMasks.BUFF_BONUS, new Object[]{Buff_Bonus_EXP}));
                // 休息獎勵經驗值
                ...
                client.announce(InfoPacket.GainEXP_Monster(gain, false, white, expmasks));
/*                if (comboKillExps.isEmpty() && comboKill > 1) {
                    client.announce(InfoPacket.monsterCombo(comboKill, mob.getObjectId()));
                }*/
            }
        }
    ...
    public void setComboKill(int mount) {
        if (mount > 0) {
            updateOneInfo(14900, "ComboK", String.valueOf(mount));
        } else {
            clearInfoQuest(14900);
        }
    }

    public int getComboKill() {
        String info = getOneInfo(14900, "ComboK");
        return info == null || info.isEmpty() ? 0 : Integer.valueOf(info);
    }
    ...
    public void forceReAddItem(Item item, MapleInventoryType type) { //used for stuff like durability, item exp/level, probably owner?
        ...
        if (type != MapleInventoryType.UNDEFINED) {
            client.announce(InventoryPacket.addInventorySlot(type, item, false));
...
//            updateDonorMedal(getInventory(MapleInventoryType.EQUIPPED).findById(1142229));
        ...
        if (pending) {
            ...
            if (pendingSkills != null) {
                ...
                for (Skill z : pendingSkills.keySet()) {
                    client.announce(CWvsContext.broadcastMsg(5, "[" + SkillFactory.getSkillName(z.getId()) + "] skill has expired and will not be available for use."));
                }
            } //not real msg
            pendingSkills = null;
            return;
    ...
    public void gainMeso(long gain, boolean show, boolean inChat) {
        if (meso + gain < 0L) {
            gain = -meso;
            meso = 0;
        } else if (meso + gain > 2147483647L) {
            gain = 2147483647L - meso;
            meso = 2147483647L;
        } else {
            meso += gain;
        }
    ...
    public int getSkillLevel(final Skill skill) {
        ...
        int linkAttack = GameConstants.getLinkedAttackSkill(skill.getId());
        if (linkAttack != skill.getId()) {
            return getSkillLevel(SkillFactory.getSkill(linkAttack));
        }
        int skillLevel;
    ...
    public void sendPolice(int greason, String reason, int duration) {
        client.announce(CWvsContext.GMPoliceMessage(String.format("You have been blocked by #bMapleGM for the %s reason.#k", "HACK")));
        ban(reason, true);
    ...
    public void sendPolice(String text) {
        client.announce(CWvsContext.GMPoliceMessage(text));
        ban(text, true);
        WorldTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
    ...
    public void sendDestroyData(MapleClient client) {
        ...
        /*
         * if (dragon != null) {
         * client.announce(CField.removeDragon(this.getId())); } if
         * (android != null) {
         * client.announce(CField.deactivateAndroid(this.getId())); }
...
//            client.announce(CField.getEffectSwitch(getId(), getEffectSwitch()));
            ...
            for (final MaplePet pet : getSummonedPets()) {
                client.announce(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), false));
...
//                client.announce(PetPacket.petStatUpdate(this));
            ...
            if (summonedFamiliar != null) {
                client.announce(CField.spawnFamiliar(summonedFamiliar, true, true));
            }
    ...
    public void unequipPet(MaplePet pet, boolean hunger) {
        if (pet.getSummoned()) {
            ...
            client.announce(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
            if (map != null) {
...
//                client.announce(CWvsContext.updateMaplePoint(maplepoints));
        ...
        if (show && quantity != 0) {
            dropMessage(-1, (quantity > 0 ? "獲得 " : "失去 ") + Math.abs(quantity) + (type == 1 ? " 樂豆點" : type == 2 ? " 楓點" : type == 3 ? " 里程" : "未知點數") + "。");
    ...
    public void addCooldown(int skillId, long startTime, long length) {
        ...
        if (isAdmin() && isInvincible()) {
            showMessage(10, "伺服器管理員無敵狀態消除技能[" + GameConstants.getLinkedAttackSkill(skillId) + "(源" + skillId + ")]，持續時間：" + length / 1000.0 + "秒");
        } else {
            if (isShowInfo()) {
                showMessage(10, "技能[" + GameConstants.getLinkedAttackSkill(skillId) + "(源" + skillId + ")]進入冷卻，持續時間：" + length / 1000.0 + "秒");
            }
            if (skillId != 131001104) {
                skillId = GameConstants.getLinkedAttackSkill(skillId);
            }
    ...
    public void giveDebuff(MapleDisease disease, int x, long duration, int skillid, int level) {
        ...
        if ((this.map != null) && (!hasDisease(disease))) {
            if ((disease != MapleDisease.誘惑) && (disease != MapleDisease.昏迷) && (disease != MapleDisease.FLAG)
            ...
            this.client.announce(CWvsContext.BuffPacket.giveDebuff(disease, x, skillid, level, (int) duration));
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM notes WHERE `to`=?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    int count = rs.getRow();
                    rs.first();
                    client.announce(CSPacket.showNotes(rs, count));
                }
            }
        } catch (SQLException e) {
    ...
    public void checkBerserk() { //berserk is special in that it doesn't use worldtimer :)
        ...
        if (skilllevel >= 1 && map != null) {
            ...
            client.announce(EffectPacket.showBuffEffect(true, this, 1320006, SpecialEffectType.LOCAL_SKILL, getLevel(), skilllevel, (byte) (stats.Berserk ? 1 : 0)));
            map.broadcastMessage(this, EffectPacket.showBuffEffect(false, this, 1320006, SpecialEffectType.LOCAL_SKILL, getLevel(), skilllevel, (byte) (stats.Berserk ? 1 : 0)), false);
        } else {
    ...
    public void removeAll(int id, boolean show) {
         ...
         * getClient().announce(CField.getShowItemGain(id,
...
//                            client.announce(PetPacket.petStatUpdate(this));
    ...
    public MapleCharacter cloneLooks() {
        ...
        ret.stack_effects.putAll(stack_effects);
        ret.dispelSummons();
        ...
        ret.client.setGmLevel(client.getGmLevel());
    ...
    public final void cloneLook() {
        ...
        for (int i = 0; i < clones.length; i++) {
            if (clones[i].get() == null) {
                ...
                newp.setName("[克隆" + (i + 1) + "號]" + newp.getName());
                map.addPlayer(newp);
    ...
    public int getMaxStats(boolean hpap) {
        ...
        if (!MapleJob.isBeginner(job)) {
            ...
            if (GameConstants.isSeparatedSp(job)) {
                jobSp = GameConstants.getSkillBookByJob(job) * (MapleJob.is龍魔導士(job) ? 3 : 5);
            } else {
                jobSp += (hpap ? 15 : 0);
            }
            total += jobSp;
        ...
        if (MapleJob.is烈焰巫師(job) || MapleJob.is暗夜行者(job)) {
            if (level <= 70) {
                total += getLevel() * 6;
            } else {
                total += 420;
...
//        client.announce(EffectPacket.TutInstructionalBalloon("Effect/OnUserEff.img/RecordClear_BT/clear"));
    ...
    public void changeChannel(final int channel) {
        ...
        client.announce(CField.getChannelChange(client, Integer.parseInt(toch.getIP().split(":")[1])));
    ...
    public void updateOneInfo(final int questid, final String key, final String value) {
        if (key == null || MapleQuest.getInstance(questid) == null/* || !MapleQuest.getInstance(questid).isPartyQuest()*/) {
            return;
        }
        if (!questinfo.containsKey(questid)) {
            if (value != null) {
        ...
        String info = changed ? newQuest.toString().substring(0, newQuest.toString().length() - 1) : newQuest.toString();
        if (info.isEmpty()) {
    ...
    public void sendEnglishQuiz(String msg) {
        client.announce(NPCPacket.getEnglishQuiz(9010000, (byte) 0, 9010000, msg, "00 00"));
    }
                        ...
                        for (MapleSummon sum : ss) {
                            if (sum.getTruePosition().distanceSq(getTruePosition()) < 400000.0 && (sum.getSkill() == 4111007 || sum.getSkill() == 4211007 || sum.getSkill() == 14111010)) {
                                if (attacke instanceof MapleMonster) {
                                    ...
                                    allDamage.add(new Pair<>(attacker.getObjectId(), allDamageNumbers));
    ...
    public void attackCarte(final MapleStatEffect eff, final int oid, final int x) {
        if (x > 0) {
            ...
            client.announce(PhantomPacket.updateCardStack(currentBattleshipHP()));
        }
    }
    ...
    public void handleForceGain(int oid, int skillid, int extraForce) {
        ...
        getClient().announce(CField.gainForce(this, oid, force, forceGain));
        if (MapleJob.is惡魔殺手(getJob())) {
            if (stats.mpRecoverProp > 0 && extraForce <= 0) {
                if (Randomizer.nextInt(100) <= stats.mpRecoverProp) {//i think its out of 100, anyway
                    force++; // counter
                    addMP(stats.mpRecover, true);
                    getClient().announce(CField.gainForce(this, oid, force, stats.mpRecover));
                }
            }
        }
    }
    ...
    public boolean getCygnusBless() {
        return getSkillLevel(MapleJob.getBeginner(job) * 10000 + 12) > 0;
    }
    ...
    public void handleCardStack(int mobid) {
        ...
        if (ceffect.makeChanceResult()) {
            ...
            this.runningStack += 1;
            this.client.announce(PhantomPacket.gainCardStack(this, this.runningStack, isAdv ? 2 : 1, ceffect.getSourceId(), mobid, 1));
            this.client.announce(PhantomPacket.updateCardStack(this.cardStack));
        }
    }
    /*
    ...
    public void chooseStolenSkill(int skillID) {
        ...
        if (stolenSkills.contains(dummy)) {
            ...
            client.announce(PhantomPacket.replaceStolenSkill(GameConstants.getStealSkill(MapleJob.getJobGrade(skillID / 10000)), skillID));
    ...
    public void removeStolenSkill(int skillID) {
        ...
        if (count >= 0) {
            ...
            for (Pair<Integer, Boolean> sk : stolenSkills) {
                if (MapleJob.getJobGrade(sk.left / 10000) == jobid) {
                    client.announce(PhantomPacket.addStolenSkill(jobid, count, sk.left, getSkillLevel(sk.left)));
                    if (sk.right) {
    ...
    public void addHonorExp(int amount, boolean show) {
        ...
        client.announce(CWvsContext.updateAzwanFame(getHonorLevel(), getHonourExp(), true));
        client.announce(CWvsContext.updateSpecialStat("honorLeveling", 0, getHonorLevel(), getHonourNextExp()));
        if (show) {
    ...
    public void honourLevelUp() {
        ...
        if (getHonorLevel() == 2 || getHonorLevel() == 30 || getHonorLevel() == 70 && inner != null && ability >= 1 && ability <= 3) {
            ...
            client.announce(CField.getCharInfo(this));
            client.announce(CField.SetField2(this));
            client.announce(CField.updateInnerPotential(ability, inner.getSkillId(), inner.getSkillLevel(), inner.getRank()));
        }
    }
    ...
    public void gainHonor(int honor, boolean show) {
        ...
        if (show) {
            client.announce(InfoPacket.showInfo(honor + " Honor EXP obtained."));
        }
    }
    ...
    public void makeNewAzwanShop() {
        ...
        client.announce(CField.getWhisper("Jean Pierre", client.getChannel(), "Psst! I got some new items in stock! Come take a look! Oh, but if your Honor Level increased, why not wait until you get a Circulator?"));
    }
    ...
    private static void addMedalString(final MapleCharacter c, final StringBuilder sb) {
        final Item medal = c.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -21/*-46*/);
        if (medal != null) { // Medal
            sb.append("<");
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM rewards WHERE `cid`=?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    //rewards.first();
                    //client.announce(Reward.updateReward(rewards.getInt("id"), (byte) 9, rewards, size, 9));
                    while (rs.next()) {
    ...
    public void toggleCustomBGState() {
        ...
        for (byte i = 0; i < 127; i++) {
            /*WriteFuture write = */client.announce(CField.removeBGLayer((getCustomBGState() == 1), 0, i, 0));
            //duration 0 = forever map 0 = current map
        }
    }
    ...
    public void updateXenonSurplus(short amount) {
        ...
        client.announce(XenonPacket.giveXenonSupply(amount));
    }
    ...
    public void handleAegisSystem(int Oid) { //處理神盾系統
        Skill skill = SkillFactory.getSkill(36110004);
        long nowtime = System.currentTimeMillis();
        if (getSkillLevel(skill) > 0 && nowtime - getstifftime() > 500) {
            ...
            if (effect != null && effect.makeChanceResult()) {
                getMap().broadcastMessage(this, SkillPacket.AccurateRocket(this, 36110004, null, Oid, 3, 3, true), true);
            }
        }
    }
    ...
    public void handleQuiverKartrige(MapleCharacter player, int oid) {
        ...
        player.getClient().announce(BuffPacket.getQuiverKartrigecount(qcount--, mode - 1));
        if (mode == 3) {
    ...
    public void handleAssassinStack(MapleMonster mob, int visProjectile) {
        ...
        if ((effect != null) && (effect.makeChanceResult()) && (mob != null)) {
            ...
            mob.setmark(false);
            this.forceCounter += 1;
            getMap().broadcastMessage(this, SkillPacket.gainAssassinStack(getId(), mob.getObjectId(), this.forceCounter, isAssassin, moboids, visProjectile, mob.getTruePosition()), true);
    ...
    public void gainExceed(int skill, short amount) {
        setExceed((short)(getExceed() + amount));
        updateExceed(skill, getExceed());
    }
    
    public void updateExceed(int skill, short amount) {
        setBuffedValue(MapleBuffStat.EXCEED, amount);
        client.announce(AvengerPacket.giveExceed(skill, amount));
    }
    ...
    public void handleExceedAttack(int skillId) {
        ...
        if (MapleJob.is惡魔復仇者(getJob())) {
            int exceedMax = getSkillLevel(31220044) > 0 ? 20 : 20;
            if (getExceed() + 1 > exceedMax) {
                setExceed((short) exceedMax);
            } else {
                gainExceed(30010230, (short) 1);
            }
        }
        ...
        if (lastExceedTime + 15000 < now) {
            ...
            exceedAttack = (short) Math.min(++exceedAttack, 4);
        }
        client.announce(AvengerPacket.giveExceed(skillId, exceedAttack));
    }
...
//            client.announce(JobPacket.DawnWarriorPacket.giveSunriseStance(getSkillLevel(skillid))); 
            ...
            dispelBuff(11101022);
//            client.announce(DawnWarriorPacket.giveEquinox_Moon(getSkillLevel(skillid), Integer.MAX_VALUE)); 
            SkillFactory.getSkill(skillid).getEffect(1).applyTo(this);
        } else if (skillid == 11121012) { // 雙重力量（旭日）
            dispelBuff(11101022);
//            client.announce(DawnWarriorPacket.giveEquinox_Sun(getSkillLevel(skillid), Integer.MAX_VALUE)); 
            SkillFactory.getSkill(skillid).getEffect(1).applyTo(this);
        }
    }
...
//        client.announce(CField.updateDeathCount(deathCount));
    }
    ...
    public void checkForceShield() {
        ...
        if (equip != null) {
            ...
            client.announce(InventoryPacket.updateEquippedItem(this, equip, (short) -10));
            equipChanged();
        }
    }
    ...
    public void checkZeroWeapon() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            forceReAddItem_NoUpdate(equip, MapleInventoryType.EQUIPPED);
            client.announce(InventoryPacket.updateEquippedItem(this, equip, (short) (i == 0 ? -11 : -10)));
        }
        equipChanged();
    }
    ...
    public void femaleMihile() {
        ...
        for (int[] item : items) {
            ...
            forceReAddItem_NoUpdate(equip, MapleInventoryType.EQUIPPED);
            client.announce(InventoryPacket.updateEquippedItem(this, equip, (short) item[1]));
        }
    ...
    public void updateSkills() {
        ...
        client.announce(CField.getKeymap(getKeyLayout(), this));
    ...
    public void monsterMultiKill() {
        if (comboKillExps.size() > 2) {
            ...
            client.announce(InfoPacket.GainEXP_MultiKill(multiKillExp, Math.min(comboKillExps.size(), 10)));
        }
        comboKillExps.clear();
    }
    ...
    public void equipSoulWeapon(Equip equip) {
        ...
        // getClient().announce(MainPacketCreator.getInventoryFull());
    ...
    public void checkSoulState(boolean useskill) {
        ...
        if (useskill) {
            if (getSoulCount() >= skill.getSoulMPCon()) {
                ...
                //if (cooldown > 0) {
                getClient().announce(CWvsContext.BuffPacket.giveSoulEffect(0));
                getMap().broadcastMessage(CWvsContext.BuffPacket.cancelForeignSoulEffect(getId()));
    ...
    public void showPlayerStats(MapleCharacter player, boolean advanced) {
        ...
        builder = new StringBuilder();
        builder.append("STR[");
        builder.append(player.getStat().getStr()).append(" + (").append(player.getStat().getTotalStr() - player.getStat().getStr()).append(")]");
        builder.append(" DEX[");
        builder.append(player.getStat().getDex()).append(" + (").append(player.getStat().getTotalDex() - player.getStat().getDex()).append(")]");
        builder.append(" INT[");
        builder.append(player.getStat().getInt()).append(" + (").append(player.getStat().getTotalInt() - player.getStat().getInt()).append(")]");
        builder.append(" LUK[");
        builder.append(player.getStat().getLuk()).append(" + (").append(player.getStat().getTotalLuk() - player.getStat().getLuk()).append(")]");
        showMessage(0, builder.toString());
        builder = new StringBuilder();
        ...
        builder.append((int) Math.ceil(((player.getStat().trueMastery) / 100.0D) * player.getStat().getCurrentMaxBaseDamage()));
        builder.append(" ~ ");
        ...
        builder = new StringBuilder();
        builder.append("暴擊發動[");
        builder.append(player.getStat().passive_sharpeye_rate());
        builder.append("%]最小暴擊[");
        builder.append(player.getStat().passive_sharpeye_min_percent());
        builder.append("%]最大暴擊[");
        builder.append(player.getStat().passive_sharpeye_percent());
        builder.append("%]總傷害[");
        ...
        builder.append("null");
        builder.append("%]移動速度[");
        builder.append(player.getStat().speed);
        builder.append("%]跳躍力[");
        builder.append(player.getStat().jump);
        builder.append("%]星光能量");
        builder.append("null");
        showMessage(0, builder.toString());
        builder = new StringBuilder();
