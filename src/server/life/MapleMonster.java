...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public int getEliteMobType() {
        return eliteMobType;
    }

    public void setEliteMobType(int eliteMobType) {
        this.eliteMobType = eliteMobType;
    }
    ...
    public final int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        if (controll != null) { // this can/should only happen when a hidden gm attacks the monster
            if (GameConstants.isAzwanMap(killer.getMapId())) {
                controll.getClient().announce(MobPacket.stopControllingMonster(this, true));
            } else {
                controll.getClient().announce(MobPacket.stopControllingMonster(this, false));
            }
    ...
    public final void switchController(final MapleCharacter newController, final boolean immediateAggro) {
        ...
        if (controllers == newController) {
            ...
            if (GameConstants.isAzwanMap(newController.getMapId())) {
                controllers.getClient().announce(MobPacket.stopControllingMonster(this, true));
            } else {
                controllers.getClient().announce(MobPacket.stopControllingMonster(this, false));
            }
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        if (GameConstants.isAzwanMap(client.getPlayer().getMapId())) {
            client.announce(MobPacket.spawnMonster(this, fake && linkCID <= 0 ? -4 : -1, 0, true));
        } else {
            client.announce(MobPacket.spawnMonster(this, fake && linkCID <= 0 ? -4 : -1, 0, false));
        }
        sendStatus(client);
    ...
    public final void sendDestroyData(final MapleClient client) {
        if (stats.isEscort() && getEventInstance() != null && lastNode >= 0) { //shammos
            ...
            if (GameConstants.isAzwanMap(client.getPlayer().getMapId())) {
                client.announce(MobPacket.killMonster(getObjectId(), 0, true));
            } else {
                client.announce(MobPacket.killMonster(getObjectId(), 0, false));
            }
            ...
            public final void run() {
                if (isAlive()) {
                    ...
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().announce(MobPacket.cancelMonsterStatus(MapleMonster.this, status));
                    }
                    stati.remove(stat);
                }
        ...
        final Runnable cancelTask = new Runnable() {
            ...
            public void run() {
                if (isAlive()) {
                    ...
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().announce(MobPacket.cancelMonsterStatus(MapleMonster.this, mse));
                    }
        ...
        if (reflection.size() > 0) {
            ...
            for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
                ...
                if (con != null) {
                    ...
                    con.getClient().announce(MobPacket.applyMonsterStatus(this, effectz));
                } else {
