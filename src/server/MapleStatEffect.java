...
//                        break;
                ...
                case 2300000:
                    ...
                    if (!mob.getStats().isBoss()) {
                        ...
                        if (absorbMp > 0) {
                            ...
                            applyto.getClient().announce(EffectPacket.showBuffEffect(true, applyto, sourceid, SpecialEffectType.LOCAL_SKILL, applyto.getLevel(), level));
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        if (isHeal() && (applyfrom.getMapId() == 749040100 || applyto.getMapId() == 749040100)) {
            applyfrom.getClient().announce(CWvsContext.enableActions());
            return false; //z
        ...
        if (primary) {
            if (info.get(MapleStatInfo.itemConNo) != 0 && !applyto.isClone() && !applyto.inPVP()) {
                if (!applyto.haveItem(info.get(MapleStatInfo.itemCon), info.get(MapleStatInfo.itemConNo), false, true)) {
                    applyto.getClient().announce(CWvsContext.enableActions());
                    return false;
                }
        ...
        if (expinc != 0) {
            ...
            applyto.getClient().announce(EffectPacket.showDodgeChanceEffect());
            ...
            if (mobid > 0) {
                ...
                applyto.getClient().announce(CWvsContext.getCard(done ? sourceid : 0, 1));
            }
            ...
            applyto.getClient().announce(CField.getPVPScore(x + bs, false));
            ...
            applyto.getClient().announce(CField.getPVPIceGage(x - info.get(MapleStatInfo.iceGageCon)));
            ...
            applyto.getClient().announce(CField.rechargeCombo(applyto.getCombo()));
            ...
            if (portal != null) {
                applyto.getClient().announce(CField.dragonBlink(portal.getId()));
            ...
            if (effectedOnEnemy > 0) {
                ...
                if (sourceid == 2910000 || sourceid == 2910001) { //red flag
                    applyto.getClient().announce(EffectPacket.showBuffEffect(true, applyto, sourceid, SpecialEffectType.MULUNG_DOJO_UP, applyto.getLevel(), level));
                    ...
                    applyto.getClient().announce(EffectPacket.showCraftingEffect("UI/UIWindow2.img/CTF/Effect", (byte) applyto.getDirection(), 0, 0));
                    ...
                    if (applyto.getTeam() == (sourceid - 2910000)) { //restore duh flag
                        ...
                        if (sourceid == 2910000) {
                            ...
                            applyto.getClient().announce(EffectPacket.showCraftingEffect("UI/UIWindow2.img/CTF/Tail/Red", (byte) applyto.getDirection(), 600000, 0));
                            ...
                            applyto.getClient().announce(EffectPacket.showCraftingEffect("UI/UIWindow2.img/CTF/Tail/Blue", (byte) applyto.getDirection(), 600000, 0));
        ...
        if (isMechDoor()) {
            ...
            applyto.getClient().announce(SkillPacket.mechPortal(door.getTruePosition()));
            if (!applyBuff) {
        ...
        if (isSoulStone() && sourceid != 24111002) {
            if (applyfrom.getParty() != null) {
                ...
                for (MapleCharacter chr : awarded) {
                    ...
                    chr.getClient().announce(EffectPacket.showBuffEffect(true, chr, sourceid, SpecialEffectType.REMOTE_SKILL, applyfrom.getLevel(), level));
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected.getId() != applyfrom.getId() && (isGmBuff() || (applyfrom.inPVP() && affected.getTeam() == applyfrom.getTeam() && Integer.parseInt(applyfrom.getEventInstance().getProperty("type")) != 0) || (applyfrom.getParty() != null && affected.getParty() != null && applyfrom.getParty().getId() == affected.getParty().getId()))) {
                    if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
                        ...
                        affected.getClient().announce(EffectPacket.showBuffEffect(true, affected, sourceid, SpecialEffectType.REMOTE_SKILL, applyfrom.getLevel(), level));
    ...
    public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
        if (sourceid == 2311005 || sourceid == 5121005 || sourceid == 1201006 || (MapleJob.isBeginner(sourceid / 10000) && sourceid % 10000 == 104)) { //doom, threaten, snatch
            ...
            chr.getClient().announce(BuffPacket.giveBuff(localsourceid, getDuration(), localstatups, this, chr));
        }
    }
    ...
    public final void applyKaiser_Combo(MapleCharacter applyto, short combo) {
        ...
        applyto.getClient().announce(CWvsContext.BuffPacket.giveBuff(0, 99999, stat, this, applyto));
    }
    ...
    public final void applyComboBuff(MapleCharacter applyto, short combo) {
        ...
        applyto.getClient().announce(CWvsContext.BuffPacket.giveBuff(this.sourceid, 99999, stat, this, applyto));
    }

    public final void applyBlackBlessingBuff(MapleCharacter applyto, int combo) {
        EnumMap stat = new EnumMap(MapleBuffStat.class);
        stat.put(MapleBuffStat.BLACK_BLESSING, combo);
        applyto.getClient().announce(CWvsContext.BuffPacket.giveBuff(this.sourceid, 0, stat, this, applyto));
        applyto.getClient().announce(CField.EffectPacket.showBlackBlessingEffect(applyto, 27100003));
    }
    ...
    public final void applyLunarTideBuff(MapleCharacter applyto) {
        ...
        applyto.getClient().announce(CWvsContext.BuffPacket.giveBuff(this.sourceid, 99999999, stat, this, applyto));
    }
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity, int targets) {
        ...
        if (infinity) {
            applyto.registerEffect(this, starttime, null, applyto.getId());
            applyto.getClient().announce(BuffPacket.giveEnergyChargeTest(0, info.get(MapleStatInfo.time) / 1000, targets));
        } else {
    ...
    public void applyReincarnation(final MapleCharacter applyfrom) {
        ...
        applyfrom.getClient().announce(CField.EffectPacket.showBuffEffect(true, applyfrom, 1320016, SpecialEffectType.LOCAL_SKILL, applyfrom.getLevel(), this.level, (byte) 0));
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            ...
            case 61121217:
                if (applyfrom.skillisCooling(61101002)) {
                    applyfrom.getClient().announce(CWvsContext.enableActions());
                    return;
                }
                if (sourceid == 61101002 && applyfrom.getTotalSkillLevel(61120007) > 0) {
                    SkillFactory.getSkill(61101002).getEffect(applyfrom.getTotalSkillLevel(61120007)).applyBuffEffect(applyfrom, applyto, primary, newDuration);
                    return;
                }
                if (applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11) != null) {
/*                    // 意志之劍的冷卻時間單獨處理
                    int cooldown = getCooldown(applyto);
                    if (cooldown > 0 && !applyto.skillisCooling(61101002)) {
                        applyto.getClient().announce(CField.skillCooldown(61101002, cooldown));
                        applyto.addCooldown(61101002, System.currentTimeMillis(), cooldown * 1000);
                    }*/
                    this.weapon = applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11).getItemId();
                    localstatups.put(MapleBuffStat.TEMPEST_BLADES, applyfrom.getSkillLevel(61101002));
                } else {
                    applyto.dropMessage(5, "佩戴的武器無法使用此技能。");
                    return;
                }
/*                if (applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11) == null) {
                    normal = false;
            ...
            case 5311004: {
                ...
                applyto.getClient().announce(CField.EffectPacket.showDiceEffect(sourceid, zz, -1, level));
                localstatups = new EnumMap<>(MapleBuffStat.class);
            ...
            case 5211007: {//dice
                ...
                applyto.getClient().announce(EffectPacket.showDiceEffect(sourceid, zz, -1, level));
                if (zz <= 1) {
                    return;
                }
                localstatups = new EnumMap<>(MapleBuffStat.class);
                localstatups.put(MapleBuffStat.DICE_ROLL, zz);
                applyto.getClient().announce(BuffPacket.giveDice(zz, sourceid, localDuration, localstatups));
                normal = false;
                showEffect = false;
                break;
            }
            ...
            case 5320007: {//dice
                ...
                applyto.getClient().announce(EffectPacket.showDiceEffect(sourceid, zz, zz2 > 0 ? -1 : 0, level));
                if (zz <= 1 && zz2 <= 1) {
                    return;
                }
            ...
            case 20031210: // 審判
                ...
                applyto.getClient().announce(CField.EffectPacket.showDiceEffect(this.sourceid, zz, -1, this.level));
                localstatups = new EnumMap(MapleBuffStat.class);
                ...
                applyto.getClient().announce(CWvsContext.BuffPacket.giveBuff(this.sourceid, localDuration, localstatups, this, applyto));
                normal = false;
                showEffect = false;
                break;
            ...
            case 15001003: {
                applyto.getClient().announce(BuffPacket.givePirate(statups, localDuration / 1000, sourceid));
                if (!applyto.isHidden()) {
            ...
            case 5220011: {// Bullseye
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().announce(BuffPacket.cancelHoming());
                    applyto.getClient().announce(BuffPacket.giveHoming(sourceid, applyto.getFirstLinkMid(), 1));
                } else {
                    return;
                }
                normal = false;
            ...
            case 2320011: //arcane aim
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().announce(BuffPacket.giveArcane(sourceid, applyto.getAllLinkMid()));
                } else {
                    return;
                }
                normal = false;
            ...
            case 110001504: {
                ...
                //stat.clear();
                //stat.clear();
                //applyto.getClient().announce(BuffPacket.giveBuff(sourceid, 2147483647, stat, this));
            ...
            case 31011001: {
                // set exceed to 0
                applyto.getClient().announce(JobPacket.AvengerPacket.cancelExceed());
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                    applyto.getClient().announce(BuffPacket.giveBuff(sourceid, localDuration, stat, this, applyto));
                    maskedStatups = new EnumMap<>(localstatups);
                    ...
                    if (isIceKnight()) {
                        ...
                        applyto.getClient().announce(BuffPacket.giveBuff(0, localDuration, stat, this, applyto));
                    }
                    final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<>(MapleBuffStat.class);
                    stat.put(MapleBuffStat.MORPH, getMorph(applyto));
                    applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                    ...
                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        localstatups.put(MapleBuffStat.MONSTER_RIDING, mountid2);
...
/*                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        //applyto.getClient().announce(BuffPacket.giveMount(mountid2, sourceid, stat));
                        applyto.getClient().announce(BuffPacket.giveBuff(sourceid, 0, stat2, this, applyto));
                        applyto.getMap().broadcastMessage(applyto, BuffPacket.showMonsterRiding(applyto.getId(), stat, mountid, sourceid), false);
                    } else {
                        return;
                    }
        ...
        if (isMechPassive()) {
            applyto.getClient().announce(EffectPacket.showBuffEffect(true, applyto, sourceid - 1000, SpecialEffectType.LOCAL_SKILL, applyto.getLevel(), level, (byte) 1));
        }
        ...
        if (normal && localstatups.size() > 0) {
            applyto.getClient().announce(BuffPacket.giveBuff((skill ? sourceid : -sourceid), localDuration, maskedStatups == null ? localstatups : maskedStatups, this, applyto));
        }
    }
