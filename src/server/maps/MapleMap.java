...
/*        if (!instanced && mapobjects.get(MapleMapObjectType.ITEM).size() >= 255) {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
                ...
                for (final MapleCharacter mc : characters) {
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8810122:
                            case 8820001:
                                mc.getClient().announce(EffectPacket.showBuffEffect(true, mc, buffid, SpecialEffectType.MULUNG_DOJO_UP, mc.getLevel(), 1)); // HT nine spirit
        ...
        if (mobid == 8810018 && mapid == 240060200) { // 闇黑龍王
            ...
            switch ((chr.getMapId() % 1000) / 100) {
                ...
                case 3:
                case 4:
                    chr.getClient().announce(CField.MapEff("monsterPark/clear"));
                    break;
                case 5:
                    if (chr.getMapId() / 1000000 == 952) {
                        chr.getClient().announce(CField.MapEff("monsterPark/clearF"));
                    } else {
                        chr.getClient().announce(CField.MapEff("monsterPark/clear"));
                    }
                    break;
    ...
    public final void spawnObtacleAtom() {
        ...
        ObtacleAtom oa = new ObtacleAtom(new Point(Randomizer.rand(getLeft(), getRight()), getBottom() - 500));
        loa.add(oa);
        broadcastMessage(CField.spawnObtacleAtomBomb(loa));
    }
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                    c.announce(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid, true));
                } else {
                    c.announce(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid, false));
                }
            }
        });
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                    c.announce(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0, true));
                } else {
                    c.announce(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0, false));
                }
            }
        });
        updateMonsterController(monster);
    ...
    public final void spawnMechDoor(final MechDoor door) {
        spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
            @Override
            public final void sendPackets(MapleClient c) {
                c.announce(SkillPacket.spawnMechDoor(door, true));
                c.announce(CWvsContext.enableActions());
            }
        });
    }
        ...
        spawnAndAddRangedMapObject(familiar, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (familiar != null && c.getPlayer() != null) {
                    c.announce(CField.spawnFamiliar(familiar, true, respawn));
                }
            }
        });
    }
...
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
        ...
        if (chr.isEquippedSoulWeapon()) {
            chr.getClient().announce(CWvsContext.BuffPacket.giveSoulGauge(chr.addgetSoulCount(), chr.getEquippedSoulSkill()));
...
//            chr.getClient().announce(packet);
        ...
        for (final MaplePet pet : chr.getSummonedPets()) {
            ...
            chr.getClient().announce(PetPacket.updatePet(pet, chr.getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), false));
            chr.getClient().announce(PetPacket.showPet(chr, pet, false, false, true));
            chr.getClient().announce(PetPacket.showPetUpdate(chr, pet.getUniqueId(), (byte) (pet.getSummonedValue() - 1)));
        }
        ...
        if (!chr.isInBlockedMap() && chr.getLevel() >= 10) {
            ...
            if (quickMove) {
                chr.getClient().announce(CField.getQuickMoveInfo(true, qmn));
            }
        }
        if (!quickMove) {
            chr.getClient().announce(CField.getQuickMoveInfo(false, new LinkedList()));
        }
        ...
        if (!chr.isClone()) {
            if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted() && !chr.isClone()) {
                if (chr.inPVP()) {
                    chr.getClient().announce(CField.getPVPClock(Integer.parseInt(chr.getEventInstance().getProperty("type")), (int) (chr.getEventInstance().getTimeLeft() / 1000)));
                } else {
            ...
            if (mapid == 914000000 || mapid == 927000000) {
                chr.getClient().announce(CWvsContext.temporaryStats_Aran());
                ...
                chr.getClient().announce(CWvsContext.temporaryStats_Reset());
            }
        }
