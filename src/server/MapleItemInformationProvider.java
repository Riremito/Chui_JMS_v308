...
public class MapleItemInformationProvider {
    ...
    public final boolean canEquip(final Map<String, Integer> stats, final int itemid, final int level, final int job, final int fame, final int str, final int dex, final int luk, final int int_, int supremacy, byte reqLevel) {
        ...
        if ((level + supremacy) >= (reqLevel != 0 ? reqLevel : stats.containsKey("reqLevel") ? stats.get("reqLevel") : 0) && 
            str >= (stats.containsKey("reqSTR") ? (MapleJob.is惡魔復仇者(job) ? 0 : stats.get("reqSTR")) : 0) && dex >= (stats.containsKey("reqDEX") ? stats.get("reqDEX") : 0) && luk >= (stats.containsKey("reqLUK") ? stats.get("reqLUK") : 0) && int_ >= (stats.containsKey("reqINT") ? stats.get("reqINT") : 0)) {
    ...
    public final Item scrollEquipWithId(final Item equip, final Item scroll, final boolean ws, final MapleCharacter chr, final int vegas) {
        if (equip.getType() == 1) { // See Item.java
            ...
            if (chr.isShowInfo()) {
                chr.showMessage(32, scrollType + " - 默認機率：" + success + "% 性向加成：" + craft + "% 幸運日加成：" + lucksKey + "% 裝備自帶卷軸100%成功率：" + equipScrollSuccess + " 最終概率：" + (equipScrollSuccess ? 100 : success) + "% 失敗消失概率：" + curse + "%");
            }

            if (success <= 0) {
                chr.dropMessage(1, "捲軸：" + scroll + " 成功機率為：" + success + " 這個捲軸可能還未修復。");
