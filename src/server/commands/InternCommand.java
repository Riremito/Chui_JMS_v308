...
//            gotomaps.put("phantom", 610010000);
                    ...
                    if (type != SearchGenerator.SearchType.未知) {
                        ...
                        switch (type) {
                            ...
                            case 臉型:
                                ...
                                c.announce(NPCPacket.getNPCTalkStyle(9010000, "", styles, false));
                                break;
                            default:
                                String str = SearchGenerator.searchData(type, search);
                                c.announce(NPCPacket.getNPCTalk(9010000, (byte) 5, str, "", (byte) 0));
                                break;
                        }
                        return 1;
                    }
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getChannelServer().manualEvent(c.getPlayer())) {
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                    ...
                    chrs.getClient().announce(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 25, c.getChannel(), "Event : MapleGM is hosting an event! Use the @joinevent command to join the event!"));
                    chrs.getClient().announce(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 26, c.getChannel(), "Event : Event Channel: " + c.getChannel() + " Event Map: " + c.getPlayer().getMap().getMapName()));
                }
            } else {
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                    //World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(22, 0, "Enteries to the GM event are closed. The event has began!"));
                    chrs.getClient().announce(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 22, c.getChannel(), "Event : Enteries to the GM event are closed. The event has began!"));
                }
            }
            return 1;
        }
    ...
    public static class ActiveBomberman extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (player.getMapId() != 109010100) {
                ...
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                    chrs.getClient().announce(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 22, c.getChannel(), "Event : Bomberman event has started!"));
                }
    ...
    public static class DeactiveBomberman extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (player.getMapId() != 109010100) {
                ...
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                    chrs.getClient().announce(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 22, c.getChannel(), "Event : Bomberman event has ended! The winners are: " + winner));
                }
            }
            return 1;
        }
    ...
    public static class 搜尋指令 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(-11, "搜尋指令(關鍵字詞:" + splitted[1] + ")結果如下:");
    ...
    public static class 全域掉寶 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (name.length() > 0) {
                c.announce(NPCPacket.getNPCTalk(9010000, (byte) 0, name.toString(), "00 00", (byte) 0, 9010000));
            } else {
