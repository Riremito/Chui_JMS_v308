...
/*            if (level > skill.getMaxLevel()) {
                ...
                for (MapleInventoryType type : MapleInventoryType.values()) {
                    for (Item item : c.getPlayer().getInventory(type)) {
                        if (ItemFlag.LOCK.check(item.getFlag())) {
                            ...
                            add = true;
                            //c.announce(CField.updateSpecialItemUse(item, type.getType()));
                        }
    ...
    public static class 發送數據包 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (builder.length() > 1) {
                c.announce(CField.getPacketFromHexString(builder.toString()));
    ...
    public static class 數據包 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length > 1) {
                c.announce(CField.getPacketFromHexString(StringUtil.joinStringFrom(splitted, 1)));
            } else {
                c.getPlayer().dropMessage(-11, "請輸入數據包數據");
            }
            return 1;
...
/*    public static class 數據包轉字符串 extends CommandExecute {
    ...
    public static class 最大楓幣 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            victim.gainMeso(2147483647L - c.getPlayer().getMeso(), true);
            return 1;
        }
    }
    ...
    public static class openUIOption extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.announce(CField.UIPacket.openUIOption(Integer.parseInt(splitted[1]), 9010000));
            return 1;
        }
    }
    ...
    public static class openUIWindow extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.announce(CField.UIPacket.openUI(Integer.parseInt(splitted[1])));
            return 1;
        }
    }
}
