...
//        rechargeableItems.add(2330016);//白金彈丸
    ...
    public void sendShop(MapleClient c) {
        ...
        c.announce(CField.NPCPacket.getNPCShop(getNpcId(), this, c));
    }

    public void sendShop(MapleClient c, int customNpc) {
        c.getPlayer().setShop(this);
        c.announce(CField.NPCPacket.getNPCShop(customNpc, this, c));
    }
    ...
    public void buy(MapleClient c, short slot, int itemId, short quantity) {
        if ((itemId / 10000 == 190) && (!GameConstants.isMountItemAvailable(itemId, c.getPlayer().getJob()))) {
            ...
            c.announce(CField.NPCPacket.confirmShopTransaction((byte) 0, this, c, -1));
            return;
        }
        ...
        if (index >= 0) {
            ...
            if ((price >= 0) && (c.getPlayer().getMeso() >= price)) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, i.getQuantity(), i.getOwner())) {
                    ...
                    c.announce(CField.NPCPacket.confirmShopTransaction((byte) 0, this, c, x));
                } else {
...
//        MapleShopItem item = findById(itemId);
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        if ((quantity <= iQuant) && (iQuant > 0)) {
            ...
            c.announce(CField.NPCPacket.confirmShopTransaction((byte) 8, this, c, -1));
        }
    }
    ...
    public void recharge(MapleClient c, byte slot) {
        ...
        if (item.getQuantity() < slotMax) {
            ...
            if (c.getPlayer().getMeso() >= price) {
                ...
                c.announce(CWvsContext.InventoryPacket.updateInventorySlot(MapleInventoryType.USE, item, false));
