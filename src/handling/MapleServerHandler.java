    ...
    public static void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c) throws Exception {
        switch (header) {
            ...
            case CLIENT_HELLO:
                ...
                break;
            case CLIENT_START:
                c.announce(LoginPacket.showMapleStory());
                break;
            case PONG:
                c.pongReceived();
                break;
...
//                System.err.println("[用戶端 報錯] 錯誤代碼 " + errortype + " 類型 " + type_str + "\r\n\t[數據] 長度 " + data_length + " [" + SendPacketOpcode.nameOf(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
            ...
            case RSA_KEY:
                break;
            case GET_SERVER:
                c.announce(LoginPacket.getLoginBackground());
                break;
            // END OF LOGIN SERVER
            case CHANGE_CHANNEL:
...
//                slea.readInt();
            ...
            case VICIOUS_HAMMER:
                slea.skip(4);
                c.announce(CSPacket.GoldenHammer((byte) 2, slea.readInt()));
                break;
...
//                CashShopOperation.doCSPackets(c);
            ...
            case TWIN_DRAGON_EGG:
                ...
                //Aristocat c.announce(CSPacket.sendTwinDragonEgg(true, true, 38, itemz, 1));
                break;
            case XMAS_SURPRISE:
            ...
            case SPECIAL_STAT:
                ...
                break;
            case UPDATE_HYPER:
                StatsHandling.DistributeHyper(slea, c, c.getPlayer(), false);
                break;
            case RESET_HYPER:
                StatsHandling.ResetHyperSP(slea, c, c.getPlayer());
                break;
            case UPDATE_HYPER_AP:
                StatsHandling.DistributeHyper(slea, c, c.getPlayer(), true);
                break;
            case RESET_HYPER_AP:
