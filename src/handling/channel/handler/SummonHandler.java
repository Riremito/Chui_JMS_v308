...
/*        byte tbyte = (slea.readByte());
        ...
        if (summon.getSkill() == 1301013 && summon.getScream()) {
            ...
            c.announce(CWvsContext.BuffPacket.giveBuff(summon.getSkill(), summon.SummonTime(360000), statups, null, null));
        }
        if (sse != null) {
...
//                if (damge > chr.getMaxDamageOver(0) && !chr.isGM()) {
        ...
        map.broadcastMessage(chr, SummonPacket.summonAttack(summon.getOwnerId(), summon.getObjectId(), animation, numAttacked/*tbyte*/, allDamage, chr.getLevel(), false), summon.getTruePosition());
        for (AttackPair attackEntry : allDamage) {
            ...
            if (totDamageToOneMonster > 0) {
                ...
                if (totDamageToOneMonster < (chr.getStat().getCurrentMaxBaseDamage() * 5.0 * (summonEffect.getSelfDestruction() + summonEffect.getDamage() + chr.getStat().getDamageIncrease(summonEffect.getSourceId())) / 100.0)) { //10 x dmg.. eh
                    ...
                    if (!mob.isAlive()) {
                        chr.getClient().announce(MobPacket.killMonster(mob.getObjectId(), 1, false));
                    }
                } else {
                    if (chr.isShowInfo()) {
    ...
    public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        switch (sum.getSkill()) {
            ...
            case 35111011: //healing
                ...
                chr.getClient().announce(EffectPacket.showBuffEffect(true, chr, sum.getSkill(), SpecialEffectType.REMOTE_SKILL, chr.getLevel(), sum.getSkillLevel()));
            ...
            case 1321007: //beholder
                ...
                chr.getClient().announce(EffectPacket.showBuffEffect(true, chr, sum.getSkill(), SpecialEffectType.REMOTE_SKILL, chr.getLevel(), bHealingLvl));
        ...
        if (GameConstants.isAngel(sum.getSkill())) {
            ...
            chr.getClient().announce(EffectPacket.showBuffEffect(true, chr, sum.getSkill(), SpecialEffectType.ZERO, 2, 1));
