...
//            FileoutputUtil.log(FileoutputUtil.Movement_Log, "NPC" + "(" + npcid + ")" + ", 封包: " + mplew.toString());
    ...
    public static final void NPCTalk(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.hasBlockedInventory()) {
            ...
            if (cm == null || (cm.getType() != ScriptType.ON_USER_ENTER && cm.getType() != ScriptType.ON_FIRST_USER_ENTER)) {
                c.getPlayer().dropMessage(-1, "你當前已經和1個NPC對話了. 如果不是請輸入 @ea 指令進行解卡。");
            }
            return;
        }
        if (npc.hasShop()) {
    ...
    public static final void QuestAction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (quest == 20734) {
            c.announce(CWvsContext.ultimateExplorer());
            return;
        }
        if (chr == null) {
            return;
        ...
        switch (action) {
            ...
            case 2: { // Complete Quest
                ...
                // c.announce(CField.completeQuest(c.getPlayer(), quest));
                //c.announce(CField.updateQuestInfo(c.getPlayer(), quest, npc, (byte)14));
                // 6 = start quest
                // 7 = unknown error
                // 8 = equip is full
                // 9 = not enough mesos
...
//                c.announce(EffectPacket.showQuetCompleteEffect());
    ...
    public static final void Storage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        switch (mode) {
            ...
            case 5: {//放入倉庫
                ...
                if (storage.isFull()) {
                    c.announce(CField.NPCPacket.getStorageFull());
                    return;
                }
            ...
            case 7: {//楓幣存取操作
                ...
                if ((meso > 0L && storageMesos >= meso) || (meso < 0L && playerMesos >= -meso)) {
                    if (meso < 0L && storageMesos - meso > 214748364799L) {//保管
                        meso = -(214748364799L - storageMesos);
                        if (-meso > playerMesos) {
                            return;
                        }
                    } else if (meso > 0L && playerMesos + meso > 2147483647L) {//取出
...
/*        final byte lastMsg = slea.readByte(); // 00 (last msg type I think)
        ...
        if (lastMsg == NPCTalk.DIRECTION_PLAYMOVE_SKIP.getType()) {
            final NPCConversationManager cm = NPCScriptManager.getInstance().getCM(c);
...
//        }
        ...
        if (lastMsg == 1) {
            ...
            if (lastMsg == NPCTalk.DIRECTION_PLAYMOVE.getType() && action == 0) {
                c.announce(CWvsContext.getTopMsg("影片播放失敗。"));
            }
            int selection = -1;
            if (slea.available() >= 4) {
                selection = slea.readInt();
        ...
        switch (mode) {
            ...
            case 1:
                if ((c.getPlayer().getRPS() == null) || (!c.getPlayer().getRPS().answer(c, slea.readByte()))) {
                    c.announce(CField.getRPSMode((byte) 13, -1, -1, -1));
                }
                break;
            case 2:
