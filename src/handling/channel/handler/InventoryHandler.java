...
public class InventoryHandler {
                ...
                if (rewards != null && rewards.getLeft() > 0) {
                    ...
                    if (itemId == 2028162) { //custom test
                        ...
                        if (unseal) {
                            ...
                            MapleInventoryManipulator.addbyItem(c, item);
                            c.announce(EffectPacket.unsealBox(item.getItemId()));
                            c.announce(EffectPacket.showRewardItemAnimation(2028162, "")); //sealed box
                        } else {
                            c.announce(CField.sendSealedBox(slot, 2028162, items)); //sealed box
                        }
                    }
                    if (itemId >= 2028154 && itemId <= 2028156
                        ...
                        if (unseal) {
                            ...
                            MapleInventoryManipulator.addbyItem(c, item);
                            c.announce(EffectPacket.unsealBox(item.getItemId()));
                            c.announce(EffectPacket.showRewardItemAnimation(itemId, ""));
                        } else {
                            c.announce(CField.sendSealedBox(slot, itemId, items));
                        }
                        return true;
                    }
                    switch (itemId) {
    ...
    public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMapId() == 749040100 || chr.getMap() == null || chr.hasDisease(MapleDisease.無法使用藥水) || chr.hasBlockedInventory() || chr.inPVP()) {
            c.announce(CWvsContext.enableActions());
            return;
        }
    ...
    public static final void UseAlienSocket(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eqq.getSocketState() != 0) { // Used before
            ...
            c.announce(CSPacket.useAlienSocket(false));
        ...
        c.announce(CSPacket.useAlienSocket(true));
    ...
    public static final void UseNebulite(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eqq.getSocket(1) == 0/* || eqq.getSocket(3) == 0 || eqq.getSocket(3) == 0*/) { // GMS removed 2nd and 3rd sockets, we can put into npc.
    ...
    public static void UseGoldenHammer(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (ItemConstants.卷軸.canHammer(equip.getItemId()) && ii.getSlots(equip.getItemId()) > 0 && equip.getViciousHammer() < 1) {
            ...
            if (Randomizer.nextInt(100) <= success) {
                ...
                c.announce(CSPacket.GoldenHammer((byte) 0, 0));
            } else {
                c.announce(CSPacket.GoldenHammer((byte) 0, 1));
            }
    ...
    public static void UsePlatinumHammer(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (ItemConstants.卷軸.canHammer(equip.getItemId()) && ii.getSlots(equip.getItemId()) > 0 && equip.getPlatinumHammer() < 5) {
            ...
            if (Randomizer.nextInt(100) <= success) {
                ...
                c.announce(CSPacket.PlatinumHammer(2));
            } else {
                c.announce(CSPacket.PlatinumHammer(3));
            }            
            c.getPlayer().forceReAddItem(equip, equip.getPosition() > 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
            MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, true);
        } else {
    ...
    public final static void UseSoulEnchanter(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {//魂卷
        ...
        if (reqLevel > socketReqLevel.getLeft() || reqLevel < socketReqLevel.getRight() || nEquip.getUpgradeSlots() > 0) {
        ...
        c.announce(InventoryPacket.scrolledItem(enchanter, eqed ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP, nEquip, false, false, eqed));
    ...
    public final static void UseSoulScroll(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {//魂珠
        ...
        c.announce(InventoryPacket.scrolledItem(soul, eqed ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP, nEquip, false, false, eqed));
    ...
    public static boolean magnifyEquip(final MapleClient c, Item magnify, Item toReveal, byte eqSlot, boolean useMagnify, boolean show) {
        ...
        if (c.getPlayer().isShowInfo()) {
            c.getPlayer().showMessage(27, "鑒定裝備：放大鏡 - " + magnify + " | 鑒定道具 - " + toReveal + " | 消耗楓幣 - " + price + " | 洞察力等級 - " + insightLevel + " | 非放大鏡鑒定 - " + !useMagnify);
        }
        ...
        if ((eqq.getState(false) < 17 && eqq.getState(false) > 0 || eqq.getState(true) < 17 && eqq.getState(true) > 0) && (((price != -1 && c.getPlayer().getMeso() >= price) || price == -1) || magnify.getItemId() == 2460005 || magnify.getItemId() == 2460004 || magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            ...
            for (int i = 1; i <= lines; i++) { // minimum 2 lines, max 5
                ...
                while (true) {
                    final int n2 = (reqLevel >= 20) ? 19 : reqLevel;
                    StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(n2);
//                    StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
            ...
            if (src == 0x7F) {
                ...
                c.announce(InventoryPacket.scrolledItem(magnify, eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED, toReveal, false, true, eqSlot < 0));
            }
    ...
    public static boolean UseUpgradeScroll(final short slot, final short dst, final short ws, final MapleClient c, final MapleCharacter chr, final int vegas, final boolean legendarySpirit) {
        ...
        if (toScroll == null || c.getPlayer().hasBlockedInventory()) {
            c.announce(CWvsContext.enableActions());
            c.announce(CField.enchantResult(0, 0));
            return false;
        }
        ...
        if (scroll == null) {
            ...
            if (scroll == null) {
                ...
                c.announce(InventoryPacket.getInventoryFull());
                c.announce(CWvsContext.enableActions());
                c.announce(CField.enchantResult(0, toScroll.getItemId()));
                return false;
            }
        }
        ...
        } else if (!ItemConstants.類型.特殊卷軸(scroll.getItemId()) && !ItemConstants.類型.白醫卷軸(scroll.getItemId()) && !ItemConstants.類型.裝備強化卷軸(scroll.getItemId()) && !ItemConstants.類型.潛能卷軸(scroll.getItemId()) && !ItemConstants.類型.附加潛能卷軸(scroll.getItemId()) && !ItemConstants.類型.回真卷軸(scroll.getItemId())) {
        ...
        if (scrollSuccess == Equip.ScrollResult.CURSE) {
            c.announce(InventoryPacket.scrolledItem(scroll, MapleInventoryType.EQUIP, toScroll, true, false, false));
            if (dst < 0) {
            ...
            if (ItemConstants.類型.回真卷軸(scroll.getItemId())) {
                c.getPlayer().forceReAddItem(scrolled, MapleInventoryType.EQUIP);
            }
            c.announce(InventoryPacket.scrolledItem(scroll, MapleInventoryType.EQUIP, scrolled, false, false, false));
        }
        ...
        c.announce(CField.enchantResult(scrollSuccess == ScrollResult.SUCCESS ? 1 : scrollSuccess == ScrollResult.CURSE ? 2 : 0, toScroll.getItemId()));
    ...
    public static void UseExpPotion(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if ((canList.size() > 0 && !ii.getCanAccountSharable(itemid).contains(job)) || ii.getCantAccountSharable(itemid).contains(job)) {
            c.announce(CWvsContext.enableActions());
            return;
        }
        ...
        c.announce(CWvsContext.updateExpPotion(last ? 0 : 2, chr.getId(), itemid, first, chr.getLevel(), limitLev[1], (exp + expForLevel) - potAllExp));
        ...
        if (success == null || Randomizer.nextInt(100) <= success) {
            ...
            if (toUse.getItemId() / 100 == 20485) {
                if (eq.getYggdrasilWisdom() > 0) {
                    ...
                    c.announce(CField.enchantResult(0, item.getItemId()));
                    c.announce(CWvsContext.enableActions());
                    return;
                }
                int minLevel = 0;
            ...
            c.announce(CField.enchantResult(1, item.getItemId()));
        } else {
            c.announce(CField.enchantResult(0, item.getItemId()));
        }
        c.announce(CWvsContext.enableActions());
    ...
    public static void UseCarvedSeal(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (Randomizer.nextInt(100) <= success) {
            ...
            if (eq.getState(false) < 17) {
                c.announce(CField.enchantResult(0, eq.getItemId()));
            ...
            c.announce(InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false, true, false));
...
////        c.getPlayer().getMap().broadcastMessage(CField.showBonusPotentialEx(c.getPlayer().getId(), suc, toUse.getItemId(), removeItem));
        c.announce(CField.enchantResult(suc ? 1 : 0, item.getItemId()));
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, toUse.getPosition(), (short) 1, false);
        c.announce(CWvsContext.enableActions());
    }
    ...
    public static void UseCube(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (used) {
            ...
            c.announce(InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, eq, false, true, false));
        }
        c.announce(CField.enchantResult(used ? 1 : 0, eq.getItemId()));
        c.announce(CWvsContext.enableActions());
        ...
        switch (type) {
            case 1: {//放置楓方塊/選擇潛能
                if (c.getPlayer().getOneInfo(GameConstants.台方塊, "u") == null) { //楓方塊
                    ...
                    if (EquipStat.EnhanctBuff.EQUIP_MARK.check(eq.getEnhanctBuff())) {
                        c.announce(CField.getShimmerCubeRespons());
                        c.announce(CWvsContext.enableActions());
                        return;
                    }
                ...
                switch (cubeId) {
                    case 5062017: {//閃耀方塊
                        ...
                        if (selected == 1) {
                            ...
                            if (EquipStat.EnhanctBuff.EQUIP_MARK.check(eq.getEnhanctBuff())) {
                                c.announce(CField.getFlashCubeRespons(cubeId, 1));
                                c.announce(CWvsContext.enableActions());
                                return;
                            }
                ...
                if (c.getPlayer().useCube(toUse.getItemId(), eq)) {
                    ...
                    switch (toUse.getItemId()) {
                        ...
                        case 5062020://閃炫方塊
                            ...
                            c.announce(CField.getShimmerCubeRespons(eq.getPotential(3, false) > 0 ? 3 : 2, newPots));
                            break;
                        default:
                            c.getPlayer().dropMessage(1, "此方塊未修復，請聯繫管理員。");
                            c.announce(CWvsContext.enableActions());
    ...
    public static void cubeMega(MapleClient c, Equip eqq, int cubeId) {
        ...
        if (c.getPlayer().isIntern()) {
            ...
            c.announce(CWvsContext.cubeMega(msg.toString(), eqq));
        } else {
    ...
    public static boolean mapleCubeAction(MapleClient c, Equip eq, boolean use, int toLock, boolean free) {
        ...
        if (eq.getLevel() == 0 && eq.getUpgradeSlots() == 0 && !ItemConstants.類型.副手(eq.getItemId()) && !ItemConstants.類型.能源(eq.getItemId()) && !ItemConstants.類型.特殊潛能道具(eq.getItemId()) || MapleItemInformationProvider.getInstance().isCash(eq.getItemId()) || ItemConstants.類型.無法潛能道具(eq.getItemId())) {
            ...
            c.announce(CField.showMapleCubeCost(3, 0));
            return false;
        }
        ...
        if (use) {
            ...
            eq.setFlag((short) (eq.getFlag() | ItemFlag.MAPLE_CUBE.getValue()));
        ...
        c.announce(CField.showMapleCubeCost(2, ItemConstants.方塊.getMapleCubeCost(mapleCubePotTimes.get(state), potentialState)));
        return true;
    }
    ...
    public static void ResetCoreAura(int slot, MapleClient c, MapleCharacter chr) {
        ...
        if ((starDust == null) || (c.getPlayer().hasBlockedInventory())) {
            c.announce(CWvsContext.InventoryPacket.getInventoryFull());
        }
    }
    ...
    public static final void useInnerCirculator(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (item.getItemId() == itemid) {
            ...
            for (InnerSkillValueHolder isvh : newValues) {
                ...
                c.announce(CField.updateInnerPotential(ability, isvh.getSkillId(), isvh.getSkillLevel(), isvh.getRank()));
                ability++;
                //c.getPlayer().changeSkillLevel(SkillFactory.getSkill(isvh.getSkillId()), isvh.getSkillLevel(), isvh.getSkillLevel());
            }
            MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);

            //c.announce(CField.gameMsg("Inner Potential has been reconfigured.")); //not sure if it's working
    ...
    public static final void UseTreasureChest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            ...
            c.announce(InfoPacket.getShowItemGain(reward, (short) amount, true));
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5050000: { // AP Reset
                ...
                switch (apto) { // AP to
                    ...
                    case 0x200: // luk
                        ...
                        break;
                    case 0x800: // hp
                        if (playerst.getMaxHp() >= 99999/*500000*/) {
                            used = false;
                        }
                        break;
                    case 0x2000: // mp
                        if (playerst.getMaxMp() >= 99999/*500000*/) {
                            used = false;
                        }
                        break;
                }
            ...
            case 5220084: {//booster pack
                ...
                c.announce(CSPacket.getBoosterPack(familiars[0], familiars[1], familiars[2]));
                c.announce(CSPacket.getBoosterPackClick());
                c.announce(CSPacket.getBoosterPackReveal());
                break;
            }
            ...
            case 5062404: { // 時裝神秘鐵砧
                ...
                if (!ItemConstants.類型.裝備(appear.getItemId()) || !ItemConstants.類型.裝備(equip.getItemId()) || appear.getItemId() / 10000 != equip.getItemId() / 10000) {
                    ...
                    c.announce(CWvsContext.getFusionAnvil(false, itemId, 2028093));
                    c.announce(CWvsContext.enableActions());
                    return;
                }
                ...
                c.announce(CWvsContext.InventoryPacket.updateSpecialItemUse((Item) equip, (byte) 1, c.getPlayer()));
                c.announce(CWvsContext.getFusionAnvil(true, itemId, 2028093));
                used = true;
                break;
            }
            ...
            case 5521000: { // 與自己分享名牌Karma
                ...
                if (item != null && ItemConstants.類型.裝備(item.getItemId()) && ((Equip) item).getKarmaCount() != 0) {
                    if (ItemFlag.KARMA_ACC.check(item.getFlag()) || ItemFlag.KARMA_ACC_USE.check(item.getFlag())) {
                        break;
                    }
                    Equip eq = (Equip) item;
            ...
            case 5520000: { // 神奇剪刀Karma
                ...
                if (item != null && ItemConstants.類型.裝備(item.getItemId()) && ((Equip) item).getKarmaCount() != 0) {
                    if (ItemFlag.KARMA_EQ.check(item.getFlag()) || ItemFlag.KARMA_USE.check(item.getFlag())) {
                        break;
                    }
                    ...
                    switch (itemId) {
                        case 5520000:
                            ...
                            break;
                        case 5520001:
                            allowKarma = ii.isPKarmaEnabled(item.getItemId()) || (ii.isKarmaEnabled(item.getItemId()) && ItemFlag.MAPLE_CUBE.check(item.getFlag()));
                            break;
                    }
            ...
            case 5570000: { // 黃金鐵鎚Vicious Hammer
                ...
                if (item != null) {
                    if (ItemConstants.卷軸.canHammer(item.getItemId()) && MapleItemInformationProvider.getInstance().getSlots(item.getItemId()) > 0 && item.getViciousHammer() < 1) {
                        ...
                        c.announce(CSPacket.ViciousHammer(true, item.getViciousHammer()));
                        used = true;
                    } else {
                        c.getPlayer().dropMessage(5, "無法使用在這個道具上。");
                        c.announce(CSPacket.ViciousHammer(true, (byte) 0));
                    }
                }
                break;
            }
...
//                World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(3, c.getChannel(), c.getPlayer().getName() + " : " + message, ear));
            ...
            case 5090100: { // 請柬-1
                ...
                if (MapleCharacterUtil.canCreateChar(sendTo, false)) { // 檢查角色名稱
                    c.announce(CSPacket.OnMemoResult((byte) 6, (byte) 1)); // 請確認接收的角色名稱。
                } else {
                    int ch = World.Find.findChannel(sendTo);
                    if (ch <= 0) { // 離線狀態
                        c.getPlayer().sendNote(sendTo, msg);
                        c.announce(CSPacket.OnMemoResult((byte) 5, (byte) 0)); // 訊息傳送成功！
                        used = true;
                    } else {
                        c.announce(CSPacket.OnMemoResult((byte) 6, (byte) 0));
                    }
                }
                break;
            }
            ...
            case 5155000: { // 卡勒塔的珍珠(更變精靈耳朵)
                ...
                if ((elf == 0 && !isMercedes) || (elf == 1 && isMercedes)) {
                    c.announce(EffectPacket.showWeirdEffect("Effect/BasicEff.img/JobChangedElf", 5155000));
                    ...
                    c.announce(EffectPacket.showWeirdEffect("Effect/BasicEff.img/JobChanged", 5155000));
            ...
            case 5156000: { // 偉大的變身之藥
                if (c.getPlayer().getMarriageId() > 0) {
                    ...
                    c.announce(CField.EffectPacket.showCraftingEffect("Effect/BasicEff.img/TransGender", (byte) 1, 0, 0));
            ...
            case 5190013: { // 開寵物商店技能
                ...
                if (zz != null && !zz.check(pet.getFlags())) {
                    ...
                    c.announce(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), false));
                    c.announce(CWvsContext.enableActions());
                    c.announce(CSPacket.changePetFlag(uniqueid, true, zz.getValue()));
                    used = true;
                }
                break;
            }
            ...
            case 5191004: { // 取消撿起無所有權道具&楓幣功能
                ...
                if (zz != null && zz.check(pet.getFlags())) {
                    ...
                    c.announce(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), false));
                    c.announce(CWvsContext.enableActions());
                    c.announce(CSPacket.changePetFlag(uniqueid, false, zz.getValue()));
                    used = true;
                }
                break;
            }
            ...
            case 5240028: { // Pet food
                ...
                if (pet.getCloseness() < 30000) {
                    ...
                    if (pet.getCloseness() >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1)) {
                        ...
                        c.announce(EffectPacket.showPetLevelUp(c.getPlayer().getPetIndex(pet)));
                        c.getPlayer().getMap().broadcastMessage(EffectPacket.showPetLevelUp(c.getPlayer(), petindex));
                    }
                }
                c.announce(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem(pet.getInventoryPosition()), false));
                ...
                if (itemId / 10000 == 512) {
                    ...
                    if (mesars > 0 && c.getPlayer().getMeso() < (Integer.MAX_VALUE - mesars)) {
                        ...
                        if (Math.random() > 0.1) {
                            ...
                            c.announce(CSPacket.sendMesobagSuccess(gainmes));
                        } else {
                            c.announce(CSPacket.sendMesobagFailed(false)); // not random
                        }
                    }
...
//                System.err.println("Return 8");
    ...
    private static boolean getIncubatedItems(MapleClient c, int itemId) {
        ...
        c.announce(CWvsContext.getPeanutResult(id1, (short) 1, id2, (short) 1, itemId));
        ...
        //c.announce(NPCPacket.getNPCTalk(1090000, (byte) 0, "黃金花生開出道具了。\r\n#i" + id1 + "##z" + id1 + "#\r\n#i" + id2 + "##z" + id2 + "#", "00 00", (byte) 0));
        return true;
    }
    ...
    public static final void OwlMinerva(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && itemid == 2310000 && !c.getPlayer().hasBlockedInventory()) {
            ...
            if (drops.length() > 0) {
                ...
                sb.append("--------------------------------------\r\n");
                sb.append(drops);
                c.announce(NPCPacket.getNPCTalk(9010000, (byte) 0, sb.toString(), "00 00", (byte) 0, 9010000));
                MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, itemid, 1, true, false);
            } else {
