...
public class MobHandler {
    ...
    public static final void MoveMonster(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        boolean useSkill = (slea.readByte() & 0xFF) > 0; // 是否使用技能
        byte mode = slea.readByte();
        int skillId = slea.readByte() & 0xFF; // 技能ID
        int skillLevel = slea.readByte(); // 技能等級
//        System.err.println(" useSkill " + useSkill + " mode = " + mode +" SkillID = " + skillId +" SkillLevel = " + skillLevel);
        short effectAfter = slea.readShort(); // 延遲時間
        int realskill = 0;
        int level = 0;
        if (useSkill) {
            byte sizee = monster.getNoSkills();
            boolean used = false;
            if (sizee > 0) {
                Pair skillToUse = monster.getSkills().get(Randomizer.nextInt(sizee));
                realskill = ((Integer) skillToUse.getLeft());
                level = ((Integer) skillToUse.getRight());
//                System.err.println("RealSkill = " + realskill +" Level = " + level);
                ...
                if (mobSkill != null && !mobSkill.checkCurrentBuff(chr, monster)) {
                    ...
                    final long LastSkillUsed = monster.getLastSkillUsed(realskill);
                    if (LastSkillUsed == 0 || currentTimeMillis - LastSkillUsed > mobSkill.getCoolTime() && !mobSkill.onlyOnce()) {
                        monster.setLastSkillUsed(realskill, currentTimeMillis, mobSkill.getCoolTime());
                        int reqHp = (int) ((float) monster.getHp() / (float) monster.getMobMaxHp() * 100.0);
        ...
        for (int i = 0; i < size1; i++) {
            List1.add(new Pair(slea.readShort(), slea.readShort()));
        }
        List List2 = new ArrayList();
        byte size = slea.readByte();
        for (int i = 0; i < size; i++) {
            List2.add(slea.readShort());
        }
        slea.skip(1);
        ...
        } else if (skillId == -1 && monster.isControllerKnowsAboutAggro() && !monster.isFirstAttack()) {
        ...
        if (MapleJob.is夜光(chr.getJob()) && (Randomizer.nextInt(100) < 20)) {
        ...
        if (res != null && res.size() > 0) {
            ...
            map.broadcastMessage(chr, MobPacket.moveMonster(useSkill, mode, realskill/*skillId*/, level/*skillLevel*/, effectAfter, monster.getObjectId(), startPos, res, List1, List2), endPos);
...
//            chr.getCheatTracker().checkMoveMonster(endPos);
    ...
    public static final void RenameFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((mf != null) && (mf.getName().equals(mf.getOriginalName())) && (MapleCharacterUtil.isEligibleCharName(newName, false))) {
            mf.setName(newName);
            c.announce(CField.renameFamiliar(mf));
        } else {
