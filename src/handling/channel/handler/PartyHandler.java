...
/*                    case 0x26: // 接受組隊邀請 / Version.176 [完成]
        ...
        switch (operation) {
                ...
                if (party == null) {
                    ...
                    if (party != null) {
                        ...
                        if ((party.getMembers().size() < 6) && (c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.組隊邀請)) == null)) {
                            ...
                            c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x1B)); // Version.176 [完成]
                        }
                    } else {
                ...
                if (theCh > 0) {
                    ...
                    if ((invited != null) && (invited.getParty() == null) && (invited.getQuestNoAdd(MapleQuest.getInstance(GameConstants.組隊邀請)) == null)) {
                        ...
                        if (party.getMembers().size() < 6) { // 邀請'(null)'加入組隊。
                            c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x21, invited.getName()));// Version.176 [完成]
                            invited.getClient().announce(CWvsContext.PartyPacket.partyInvite(c.getPlayer()));
                        } else { // 組隊成員已滿
                            c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x1B)); // Version.176 [完成]
                        }
                    } else { // 已有隊伍
                        c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x1A)); // Version.176 [完成]
                    }
                } else { // 找不到玩家
                    c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x38)); // Version.176 [完成]
                }
                break;
            case 6: // 強制退出
            ...
            case 8: // 申請隊伍
                ...
                if ((cfrom != null) && (cfrom.getQuestNoAdd(MapleQuest.getInstance(GameConstants.組隊請求)) == null)) {
                    c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x43, c.getPlayer().getName())); // Version.176 [完成]
                    cfrom.getClient().announce(CWvsContext.PartyPacket.partyRequestInvite(c.getPlayer()));
                } else {
    ...
    public static void MemberSearch(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.announce(CWvsContext.PartyPacket.showMemberSearch(members));
    }
    ...
    public static final void PartySearch(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.announce(CWvsContext.PartyPacket.showPartySearch(parties));
    }
        ...
        switch (mode) {
            ...
            case 159:
                ...
                if ((c.getPlayer().getParty() == null) && (World.Party.searchParty(pst).size() < 10)) {
                    ...
                    if (pst.exped) {
                        c.announce(CWvsContext.ExpeditionPacket.expeditionStatus(World.Party.getExped(party.getExpeditionId()), true, false));
                    }
                    c.announce(CWvsContext.PartyPacket.partyListingAdded(ps));
                } else {
            ...
            case 161:
                ...
                c.announce(CWvsContext.PartyPacket.getPartyListing(pst));
                break;
            case -102:
            case -94:
            case 84:
            ...
            case 163:
                ...
                if (party != null) {
                    ...
                    if ((ps != null) && (c.getPlayer().getLevel() <= ps.getType().maxLevel) && (c.getPlayer().getLevel() >= ps.getType().minLevel) && (party.getMembers().size() < 6)) {
                        ...
                        c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x17));//0x15+2 175
                    }
                } else {
                    ...
                    if (exped != null) {
                        ...
                        if ((ps != null) && (c.getPlayer().getLevel() <= ps.getType().maxLevel) && (c.getPlayer().getLevel() >= ps.getType().minLevel) && (exped.getAllMembers() < exped.getType().maxMembers)) {
                            ...
                            if (partyId < 0) {
                                c.announce(CWvsContext.PartyPacket.partyStatusMessage(0x15));//0x15+2 175
                            } else if (partyId == 0) {
                                party = World.Party.createPartyAndAdd(partyplayer, exped.getId());
                                c.getPlayer().setParty(party);
                                c.announce(CWvsContext.PartyPacket.partyCreated(party));
                                c.announce(CWvsContext.ExpeditionPacket.expeditionStatus(exped, true, false));
                            ...
                            c.announce(CWvsContext.ExpeditionPacket.expeditionError(0, c.getPlayer().getName()));
                        }
                    }
                }
                break;
        ...
        switch (mode) {
            ...
            case 134:
                ...
                if ((et != null) && (c.getPlayer().getParty() == null) && (c.getPlayer().getLevel() <= et.maxLevel) && (c.getPlayer().getLevel() >= et.minLevel)) {
                    ...
                    c.announce(CWvsContext.ExpeditionPacket.expeditionError(0, ""));
                }
                break;
            case 77://65
            case 135:
                ...
                if (theCh > 0) {
                    ...
                    if ((invited != null) && (invited.getParty() == null) && (party != null) && (party.getExpeditionId() > 0)) {
                        ...
                        if ((me != null) && (me.getAllMembers() < me.getType().maxMembers) && (invited.getLevel() <= me.getType().maxLevel) && (invited.getLevel() >= me.getType().minLevel)) {
                            c.announce(CWvsContext.ExpeditionPacket.expeditionError(7, invited.getName()));
                            invited.getClient().announce(CWvsContext.ExpeditionPacket.expeditionInvite(c.getPlayer(), me.getType().exped));
                        } else {
                            c.announce(CWvsContext.ExpeditionPacket.expeditionError(3, invited.getName()));
                        }
                    } else {
                        c.announce(CWvsContext.ExpeditionPacket.expeditionError(2, name));
                    }
                } else {
                    c.announce(CWvsContext.ExpeditionPacket.expeditionError(0, name));
                }
                break;
            case 78://66
            case 136:
                    ...
                    if ((exped != null) && (action == 8)) {
                        if ((c.getPlayer().getLevel() <= exped.getType().maxLevel) && (c.getPlayer().getLevel() >= exped.getType().minLevel) && (exped.getAllMembers() < exped.getType().maxMembers)) {
                            ...
                            if (partyId < 0) {
                                ...
                                c.getPlayer().receivePartyMemberHP();
                                c.getPlayer().updatePartyMemberHP();
                                c.announce(CWvsContext.ExpeditionPacket.expeditionStatus(exped, false, false));
                                World.Party.expedPacket(exped.getId(), CWvsContext.ExpeditionPacket.expeditionJoined(c.getPlayer().getName()), null);
                            }
                        } else {
                            c.announce(CWvsContext.ExpeditionPacket.expeditionError(3, cfrom.getName()));
                        }
                    } else if (action == 9) {
                        cfrom.getClient().announce(CWvsContext.PartyPacket.partyStatusMessage(0x19, c.getPlayer().getName()));//0x17+2 175
                    }
                }
                break;
            case 79://67
                                ...
                                if ((expelled != null) && (expelled.isOnline())) {
                                    ...
                                    if (partyIndexTo < exped.getParties().size()) {
                                        ...
                                        if ((party != null) && (party.getMembers().size() < 6)) {
                                            ...
                                            chr.getClient().announce(CWvsContext.ExpeditionPacket.expeditionStatus(exped, true, false));
                                        }
                                    } else {
                                        MapleParty party = World.Party.createPartyAndAdd(expelled, exped.getId());
                                        chr.setParty(party);
                                        chr.getClient().announce(CWvsContext.PartyPacket.partyCreated(party));
                                        chr.getClient().announce(CWvsContext.ExpeditionPacket.expeditionStatus(exped, true, false));
                                        World.Party.expedPacket(exped.getId(), CWvsContext.ExpeditionPacket.expeditionUpdate(exped.getIndex(party.getId()), party), null);
                                    }
                                    if ((c.getPlayer().getEventInstance() != null)
