...
//            }
            ...
            if (MapleJob.is天使破壞者(c.getPlayer().getJob())) {
                ...
                if (sn > 0) {
                    if (rand) {
                        c.announce(JobPacket.AngelicPacket.SoulSeekerRegen(c.getPlayer(), sn));
                    }
                }
            }
...
//        c.announce(CWvsContext.enableActions()); // 找不到cancelchair包頭暫時用這個替代
    }
    ...
    public static void TrockAddMap(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        c.announce(CSPacket.OnMapTransferResult(chr, vip, addrem == 0));
    }
        ...
        switch (mode) {
            ...
            case 0x1E: // Warn
                ...
                if (target != null) {
                    ...
                    c.announce(CField.getGMEffect(0x1E, (byte) 1));
                } else {
    ...
    public static void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((chr.getStat().dodgeChance > 0) && (Randomizer.nextInt(100) < chr.getStat().dodgeChance)) {
            c.announce(CField.EffectPacket.showDodgeChanceEffect());
            return;
        }
    ...
    public static void AranCombo(MapleClient c, MapleCharacter chr, int toAdd) {
        if ((chr != null) && (chr.getJob() >= 2000) && (chr.getJob() <= 2112)) {
            ...
            c.announce(CField.updateCombo(combo));
    ...
    public static void CancelBuffHandler(int sourceid, MapleCharacter chr) {
        ...
        if (GameConstants.isExceedAttack(sourceid)) {
            chr.getClient().announce(JobPacket.AvengerPacket.cancelExceedAttack());
        }
    ...
    public static void releaseTempestBlades(LittleEndianAccessor slea, MapleCharacter chr) {
        if (!chr.isAlive() || chr.skillisCooling(61101002)) {
...
//        chr.getMap().broadcastMessage(CField.gainForce(false, chr, oids, 2, skillid, forceinfo));
        chr.getClient().announce(CField.gainForce(false, chr, oids, 2, skillid, forceinfo));
...
/*        MapleStatEffect skill_effect = SkillFactory.getSkill(skillid).getEffect(chr.getTotalSkillLevel(skillid));
        ...
        chr.cancelEffectFromBuffStat(MapleBuffStat.TEMPEST_BLADES);
        // 意志之劍的冷卻時間單獨處理
        if (!chr.skillisCooling(61101002)) {
            chr.getClient().announce(CField.skillCooldown(61101002, 20));
            chr.addCooldown(61101002, System.currentTimeMillis(), 20 * 1000);
        }
    }
    ...
    public static void KaiserQuickKey(LittleEndianAccessor slea, MapleCharacter chr) {
        if (MapleJob.is凱撒(chr.getJob())) {
            ...
            chr.getClient().announce(CField.sendKaiserQuickKey(skills));
        }
    }

    public static void QuickSlot(LittleEndianAccessor slea, MapleCharacter chr) {
        if ((slea.available() == 32/*114*/) && (chr != null)) {
//            slea.readShort();
...
//            }
//            c.announce(JobPacket.AngelicPacket.SoulSeeker(chr, skillid, soulnum, scheck, scheck2));
...
//        }
        ...
        if ((skill == null) || ((GameConstants.isAngel(skillid)) && (chr.getStat().equippedSummon / 10000000 == 8 ? chr.getStat().equippedSummon != skillid : chr.getStat().equippedSummon % 10000 != skillid % 10000)) || ((chr.inPVP()) && (skill.isPVPDisabled()))) {
            c.announce(CWvsContext.enableActions());
            return;
        }
        ...
        if ((MapleJob.is幻影俠盜(chr.getJob())) && (!MapleJob.is幻影俠盜(skillid / 10000))) {
            ...
            if (skillJob % 100 == 0) {
                checkSkilllevel = 24001001;
            } else if (skillJob % 10 == 0) {
                checkSkilllevel = 24101001;
            } else if (skillJob % 10 == 1) {
                checkSkilllevel = 24111001;
            } else {
                checkSkilllevel = 24121001;
            }
        }
        ...
        if (chr.isShowInfo()) {
            chr.showMessage(27, "[SpecialSkill] - 技能ID：" + skill.getName() + "(" + skillid + ") 技能等級：" + skillLevel);
            if (GameConstants.getLinkedAttackSkill(skillid) != skillid) {
                chr.showMessage(27, "[SpecialSkill] - 連接技能ID：" + GameConstants.getLinkedAttackSkill(skillid) + " 連接技能等級：" + checkSkilllevel);
            }
        }
        if (checkSkilllevel <= 0 || checkSkilllevel != skillLevel) {
            if ((!GameConstants.isMulungSkill(skillid)) && (!GameConstants.isPyramidSkill(skillid)) && (chr.getTotalSkillLevel(GameConstants.getLinkedAttackSkill(skillid)) <= 0) && !GameConstants.isAngel(skillid)) {
                if (chr.isShowErr()) {
                    chr.showMessage(28, new StringBuilder().append("[SpecialSkill] 使用技能出現異常 技能ID：").append(skillid).append(" 角色技能等級：").append(checkSkilllevel).append(" 封包獲取等級：").append(skillLevel).append(" 是否相同：").append(checkSkilllevel == skillLevel).toString());
                }
                c.announce(CWvsContext.enableActions());
                return;
            }
        ...
        switch (skillid) {
            ...
            case 12101025: // 火球連結
                ...
                if (s == 1) {
                    c.announce(CField.instantMapWarp(2, 0, chr.getId(), pos));
                }
                break;
            case 20031210: // 審判
                c.announce(JobPacket.PhantomPacket.gainCardStack(chr, 0, 24120002, 2, 1, 10));
            ...
            case 110001504: // 召喚貓咪
                ...
                c.announce(CWvsContext.BuffPacket.giveBuff(skillid, 2100000000, stat, null, chr));
                c.announce(CWvsContext.enableActions());
                break;
//            case 11101022:
//            case 11111022:
        ...
        if (MapleJob.is天使破壞者(chr.getJob())) {
            ...
            if (Recharge > -1) {
                if (Randomizer.isSuccess(Recharge)) {
                    c.announce(AngelicPacket.unlockSkill());
                    c.announce(CField.EffectPacket.showRechargeEffect());
                    if (c.getPlayer().isGM()) {
                        c.announce(AngelicPacket.unlockSkill());
//                        c.announce(AngelicPacket.showRechargeEffect());
                    } else {
                        c.announce(AngelicPacket.lockSkill(skillid));
                    }
                }
                c.announce(CWvsContext.enableActions());
            } else {
...
//                    c.announce(AngelicPacket.showRechargeEffect());
        ...
        if (skillid == 2001009) {
            if (chr.getBuffSource(MapleBuffStat.CHILLING_STEP) == 2201009) {
                ...
                c.announce(CField.spawnSpecial(2201009, newUserPos.x, newUserPos.y, newUserPos.x + 179, newUserPos.y + 21));
            }
        }
    }
    ...
    public static void ArrowBlasterAction(LittleEndianAccessor slea, final MapleClient c, MapleCharacter chr) {
        ...
        c.announce(CField.spawnArrowBlaster(chr, x, y, a));
        c.announce(CField.controlArrowBlaster(a));
        BuffTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                c.announce(CField.cancelArrowBlaster(a));
            }
        }, 30000L);
    }

    public static boolean doNotEffectforAttack(int skill) {
        switch (skill) {
            case 2201009:
            case 4221052:
            case 95001000:
    ...
    public static void closeRangeAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr, final boolean passive) {
        //獲取攻擊訊息
        ...
        if (!dmg.isEmpty()) {
//            chr.dropMessage(-1, "傷害: " + dmg);//debug mode
        }
...
//                    c.announce(AngelicPacket.showRechargeEffect());
                            ...
                            c.announce(AngelicPacket.lockSkill(attack.skill));
                        }
                    }
                } else {
                    if (c.getPlayer().isGM()) {
...
//                    c.announce(AngelicPacket.showRechargeEffect());
            ...
            if (attack.skill == 4100012 || attack.skill == 4120019) {
                maxdamage *= (effect.getDamage() + chr.getStat().getDamageIncrease(attack.skill) + effect.getX() * chr.getLevel()) / 100.0D;
            } else {
...
/*                if (chr.skillisCooling(attack.skill) && doNotEffectforAttack(attack.skill)) {
            ...
            if (attack.skill == 15121001) {
                attackCount += Math.min(chr.getBuffedValue(MapleBuffStat.INDIE_IGNORE_MOB_PDP_R) / 5, chr.getStat().raidenCount);
            ...
            if (attack.skill == 131001010 || attack.skill == 131001011) {
                ...
                Integer value = chr.getBuffedValue(MapleBuffStat.PINK_BEAN_YOYO);
                if (value == null || value <= 0) {
                    chr.dropMessage(5, "沒有準備好的溜溜球所以無法使用技能。");
                    c.announce(CWvsContext.enableActions());
                    return;
                }
                chr.setBuffedValue(MapleBuffStat.PINK_BEAN_YOYO, Math.max(0, --value));
            }
        }
        ...
        byte[] packet;
        if (passive) {
            packet = CField.passiveAttack(chr, skillLevel, 0, attack, hasMoonBuff);
        } else {
            packet = CField.closeRangeAttack(chr, skillLevel, 0, attack, hasMoonBuff);
        }
        if (!chr.isHidden()) {
        ...
        for (int i = 0; i < clones.length; i++) {
            if (clones[i].get() != null) {
                ...
                if (passive) {
                    packetClone = CField.passiveAttack(clone, skillLevel2, 0, attack2, hasMoonBuff);
                } else {
                    packetClone = CField.closeRangeAttack(clone, skillLevel2, 0, attack2, hasMoonBuff);
                }
...
//                    c.announce(AngelicPacket.showRechargeEffect());
                ...
                if ((chr.getJob() == 412) && (bulletConsume > 0) && (ipp.getQuantity() < MapleItemInformationProvider.getInstance().getSlotMax(projectile))) {
                    ...
                    if (chr.getTotalSkillLevel(expert) > 0) {
                        ...
                        if (eff.makeChanceResult()) {
                            ...
                            c.announce(CWvsContext.InventoryPacket.updateInventorySlot(MapleInventoryType.USE, ipp, false));
                            bulletConsume = 0;
                            c.announce(CWvsContext.InventoryPacket.getInventoryStatus());
                        }
                    }
                }
    ...
    public static void MoveAndroid(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        slea.skip(12);
        List res = MovementParse.parseMovement(slea, 3, chr);
    ...
    public static void MovePlayer(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        slea.skip(33);
        final Point Original_Pos = slea.readPos();
        slea.readInt();
        if (chr == null) {
            return;
        }
...
//            slea.readInt();
            ...
            if ((targetid != -1) && (!chr.isAlive())) {
                ...
                if (!wheel) {
                    ...
                    c.announce(CField.EffectPacket.useAmulet(2, (byte) (chr.getInventory(MapleInventoryType.CASH).countById(5510000) - 1), (byte) 0));
...
//        boolean autoSave = true;
    ...
    public static void LoadPlayerSuccess(MapleClient c, MapleCharacter chr) {
        ...
        String msg = "歡迎來到「#b" + c.getChannelServer().getServerName() + "#k」\r\n\r\n祝您遊戲愉快！";
        c.announce(CField.sendHint(msg, 200, 5));
//        if (!GameConstants.isTutorialMap(chr.getMapId())) {
//            c.announce(CField.startMapEffect("歡迎來到「" + c.getChannelServer().getServerName() + "」，祝您遊戲愉快！", 5122000, true));
//        }
        if (GameConstants.isTutorialMap(chr.getMapId())) {
            chr.dropMessage(5, "你的經驗倍率將會被設置為1倍，直到您完成出生劇情為止。");
        }
        chr.dropMessage(5, "在'快速移動'裡的'多功能便捷NPC'有很多功能哦，有空可以看看！");
        chr.dropMessage(5, "在'快速移動'裡還有能傳送到任意城市的'次元傳送門'！");
...
//        chr.iNeedSystemProcess();
    ...
    public static class ZeroHandler {
        ...
        public static void talkZeroNpc(final LittleEndianAccessor slea, final MapleClient c) {
            c.announce(CField.ZeroPacket.NPCTalk());
            c.announce(CWvsContext.enableActions());
        }
...
/*        if (!c.getPlayer().isHidden()) {
            c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.getEffectSwitch(c.getPlayer().getId(), c.getPlayer().getEffectSwitch()), true);
        } else {
            c.getPlayer().getMap().broadcastGMMessage(c.getPlayer(), CField.getEffectSwitch(c.getPlayer().getId(), c.getPlayer().getEffectSwitch()), true);
        }*/
    }
    ...
    public static void UpdatePinkbeenYoyo(final MapleClient c) {
        ...
        if (value != null) {
            final long start = c.getPlayer().getBuffedStarttime(MapleBuffStat.PINK_BEAN_YOYO);
            if ((System.currentTimeMillis() - start) < 7000 || value >= 8) {
                return;
            }
            chr.setBuffedValue(MapleBuffStat.PINK_BEAN_YOYO, Math.min(8, ++value));
        }
        effect.applyTo(chr);
    }
