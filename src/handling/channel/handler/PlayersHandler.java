...
public class PlayersHandler {
    ...
    public static void DressUpRequest(final MapleCharacter chr, LittleEndianAccessor slea) {
        ...
        switch (code) {
            case 5010093:
                chr.getClient().announce(JobPacket.AngelicPacket.updateDress(code, chr));
                chr.getClient().announce(CField.updateCharLook(chr, true));
                break;
            case 5010094:
                chr.getClient().announce(JobPacket.AngelicPacket.updateDress(code, chr));
                chr.getClient().announce(CField.updateCharLook(chr, true));
    ...
    public static final void TouchRune(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (rune != null && rune.getRuneType() == type) {
            if (chr.getKeyValue("LastTouchedRune") != null && chr.getRuneTimeStamp() > System.currentTimeMillis()) {
                chr.getClient().announce(CField.RunePacket.RuneAction(2, (int) (chr.getRuneTimeStamp() - System.currentTimeMillis())));
                chr.getClient().announce(CWvsContext.enableActions());
                return;
            }
            chr.setTouchedRune(type);
            chr.getClient().announce(CField.RunePacket.RuneAction(7/*5*/, 0));
        } else {
        ...
        if (result == 1) {
            switch (chr.getTouchedRune()) {
                case 0: //疾速之輪
                    ...
                    effect.applyTo(chr);
                    chr.getClient().announce(CWvsContext.getMapleTipMsg("疾速之輪:提升所有速度獲得追加經驗值."));
                    break;
                case 1: //再生之輪
                    effect = SkillFactory.getSkill(80001428).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().announce(CWvsContext.getMapleTipMsg("再生之輪: 提升再生力獲得追加經驗值."));
                    break;
                case 2: //崩壞之輪
                    effect = SkillFactory.getSkill(80001430).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().announce(CWvsContext.getMapleTipMsg("崩壞之輪: 強化攻擊力獲得追加經驗值.."));
                    break;
                case 3: //破滅之輪
                    effect = SkillFactory.getSkill(80001432).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().announce(CWvsContext.getMapleTipMsg("破滅之輪: 強化攻擊力獲得追加經驗值."));
                    break;
            }
    ...
    public static void FollowRequest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (tt != null && tt.getPosition().distanceSq(c.getPlayer().getPosition()) < 10000 && tt.getFollowId() == 0 && c.getPlayer().getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) { //estimate, should less
            ...
            tt.getClient().announce(CWvsContext.followRequest(c.getPlayer().getId()));
        } else {
            c.announce(CWvsContext.broadcastMsg(1, "You are too far away."));
        }
    }
    ...
    public static void DoRing(final MapleClient c, final String name, final int itemid) {
        ...
        if (errcode > 0) {
            c.announce(SkillPacket.sendEngagement((byte) errcode, 0, null, null));
            c.announce(CWvsContext.enableActions());
            return;
        }
        c.getPlayer().setMarriageItemId(itemid);
        /*WriteFuture write = */chr.getClient().announce(SkillPacket.sendEngagementRequest(c.getPlayer().getName(), c.getPlayer().getId()));
    }
    ...
    public static void RingAction(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 0) {
            ...
            if (c.getPlayer().getMarriageId() > 0 || chr == null || chr.getId() != id || chr.getMarriageItemId() <= 0 || !chr.haveItem(chr.getMarriageItemId(), 1) || chr.getMarriageId() > 0 || !chr.isAlive() || chr.getEventInstance() != null || !c.getPlayer().isAlive() || c.getPlayer().getEventInstance() != null) {
                c.announce(SkillPacket.sendEngagement((byte) 0x1D, 0, null, null));
                c.announce(CWvsContext.enableActions());
                return;
            }
            if (accepted) {
                ...
                if (!MapleInventoryManipulator.checkSpace(c, newItemId, 1, "") || !MapleInventoryManipulator.checkSpace(chr.getClient(), newItemId, 1, "")) {
                    c.announce(SkillPacket.sendEngagement((byte) 0x15, 0, null, null));
                    c.announce(CWvsContext.enableActions());
                    return;
                }
                try {
                    ...
                    chr.getClient().announce(SkillPacket.sendEngagement((byte) 0x10, newItemId, chr, c.getPlayer()));
                ...
                chr.getClient().announce(SkillPacket.sendEngagement((byte) 0x1E, 0, null, null));
            }
            c.announce(CWvsContext.enableActions());
    ...
    public static void Report(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (other == null || type == null || other.isIntern()) {
            c.announce(CWvsContext.report(4));
            return;
        }
        ...
        if (theTime + 7200000 > currentTime && !c.getPlayer().isIntern()) {
            ...
            c.announce(CWvsContext.report(2));
        }
    }
    ...
    public static void claimSilentCrusadeReward(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (use < 1 || setup < 1 || etc < 1) {
            c.announce(CWvsContext.getSilentCrusadeMsg((byte) 2));
            c.announce(CWvsContext.enableActions());
            return;
        }
        switch (chapter) {
    ...
    public static void buySilentCrusade(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (tokenPrice == 0) {
            ...
            c.announce(CWvsContext.getSilentCrusadeMsg((byte) 3));
            c.announce(CWvsContext.enableActions());
            return;
        }
        if (c.getPlayer().getInventory(GameConstants.getInventoryType(itemId)).getNumFreeSlot() >= quantity) {
            if (c.getPlayer().itemQuantity(4310029) < tokenPrice) {
                c.announce(CWvsContext.getSilentCrusadeMsg((byte) 1));
                c.announce(CWvsContext.enableActions());
                return;
            }
            if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                ...
                if (itemId < 2000000 && potentialGrade > 0) {
                    ...
                    if (!MapleInventoryManipulator.addById(c, itemId, quantity, "BUY_SILENT_CRUSADE, 時間:" + FileoutputUtil.CurrentReadable_Time())) {
                        c.announce(CWvsContext.getSilentCrusadeMsg((byte) 2));
                        c.announce(CWvsContext.enableActions());
                        return;
                    }
                }
                c.announce(CWvsContext.getSilentCrusadeMsg((byte) 0));
                c.announce(CWvsContext.enableActions());
            } else {
                c.announce(CWvsContext.getSilentCrusadeMsg((byte) 2));
                c.announce(CWvsContext.enableActions());
        ...
        if (mode == 1) {
            if (c.getPlayer().getQuestStatus(GameConstants.PLAYER_INFORMATION) > 0) {
                try {
                    ...
                    c.announce(CWvsContext.loadInformation((byte) 2, Integer.parseInt(info[0]), Integer.parseInt(info[1]), Integer.parseInt(info[2]), Integer.parseInt(info[3]), true));
                } catch (NumberFormatException ex) {
                    c.announce(CWvsContext.loadInformation((byte) 4, 0, 0, 0, 0, false));
    ...
    public static void FindFriends(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) {
            case 5:
                if (c.getPlayer().getQuestStatus(GameConstants.PLAYER_INFORMATION) == 0) {
                    c.announce(CWvsContext.findFriendResult((byte) 6, null, 0, null));
                    c.announce(CWvsContext.enableActions());
                    return;
                }
            case 7:
                ...
                if (characters.isEmpty()) {
                    c.announce(CWvsContext.findFriendResult((byte) 9, null, 12, null));
                } else {
                    c.announce(CWvsContext.findFriendResult((byte) 8, characters, 0, null));
                }
                break;
        }
    }
    ...
    public static void MonsterBookInfoRequest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (player != null && !player.isClone()) {
            if (!player.isGM() || c.getPlayer().isGM()) {
                c.announce(CWvsContext.getMonsterBookInfo(player));
            }
        }
    }
    ...
    public static void LeaveAzwan(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.announce(CField.showEffect("hillah/fail"));
        c.announce(CField.UIPacket.openUIOption(0x45, 0));
    ...
    public static void 王圖復活(LittleEndianAccessor slea, MapleClient c) {
        if (c.getPlayer() == null) {
            c.announce(CWvsContext.enableActions());
            return;
        }
        ...
        if (mode == 2) {
            ...
            for (byte inv = 1; inv <= 5; inv++) {
                if (c.getPlayer().getInventory(MapleInventoryType.getByType(inv)).getNumFreeSlot() < 2) {
                    c.announce(CWvsContext.magicWheel((byte) 7, null, null, 0));
                    c.announce(CWvsContext.enableActions());
                    return;
                }
            }
            ...
            if (!MapleInventoryManipulator.removeFromSlot_Lock(c, GameConstants.getInventoryType(tokenId), toUseSlot, (short) 1, false, false)) {
                c.announce(CWvsContext.magicWheel((byte) 9, null, null, 0));
                c.announce(CWvsContext.enableActions());
                return;
            }
            c.announce(CWvsContext.magicWheel((byte) 3, items, data, end));
        } else if (mode == 4) {
            ...
            if (item == 0 || !MapleInventoryManipulator.addById(c, item, (short) 1, null)) {
                c.announce(CWvsContext.magicWheel((byte) 0xA, null, null, 0));
                c.announce(CWvsContext.enableActions());
                return;
            }
            //} catch (Exception ex) {
            ...
            c.announce(CWvsContext.magicWheel((byte) 5, null, null, 0));
        }
    }
...
//        System.err.println("onReward");
        ...
        if (mode == 2) { // 拒絕禮物
            ...
            if (reward == null) {
                c.announce(Reward.receiveReward(id, (byte) 0x15, 0));
                c.announce(CWvsContext.enableActions());
                return;
            }
            ...
            switch (type) {
                ...
                case 4: // 楓幣
                    ...
                    break;
                case 5: // 經驗值
                    int maxlevel = MapleJob.is皇家騎士團(c.getPlayer().getJob()) ? 120 : 200;
                    ...
                    break;
                default:
                    System.out.println("未知類型: " + type);
                    break;
            }
            c.announce(Reward.receiveReward(id, msg, quantity));
        }
        if (mode < 0 || mode > 2) {
            System.out.println("未知 Mode: " + mode);
        }
    }
    ...
    public static void viewSkills(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (!c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkills().isEmpty() && MapleJob.is冒險家(jobid)) {
            c.announce(CField.viewSkills(c.getChannelServer().getPlayerStorage().getCharacterById(victim)));
        } else {
    ...
    public static void AttackPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        final boolean aran = weapon != null && weapon.getItemId() / 10000 == 144 && MapleJob.is狂狼勇士(chr.getJob());
        ...
        int visProjectile = 0;
        if ((chr.getJob() >= 3500 && chr.getJob() <= 3512) || MapleJob.is蒼龍俠客(chr.getJob())) {
            visProjectile = 2333000;
        } else if (MapleJob.is重砲指揮官(chr.getJob())) {
            visProjectile = 2333001;
        } else if (!MapleJob.is精靈遊俠(chr.getJob()) && chr.getBuffedValue(MapleBuffStat.SOULARROW) == null && slot > 0) {
    ...
    public static void updateSpecialStat(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (stat) {
            case "honorLeveling":
                c.announce(CWvsContext.updateSpecialStat(stat, array, mode, c.getPlayer().getHonourNextExp()));
                break;
            case "hyper":
                c.announce(CWvsContext.updateSpecialStat(stat, array, mode, 0));
                break;
        }
    }
    ...
    public static void UseChronosphere(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (GameConstants.isBossMap(toMapId)) {
            c.announce(CSPacket.getTrockMessage((byte) 11));
            c.announce(CWvsContext.errorChronosphere());
            return;
        }
        ...
        if (moveTo == null) {
            ...
            if (chr.getFreeChronosphere() > 0) {
                ...
                chr.dropMessage(5, "你使用了免費強化任意門從\"" + chr.getMap().getMapName() + "\"傳送到\"" + moveTo.getMapName() + "\"本週剩餘免費強化任意門：" + chr.getFreeChronosphere() + "。");
                ...
                chr.dropMessage(5, "你使用了強化任意門從\"" + chr.getMap().getMapName() + "\"傳送到\"" + moveTo.getMapName() + "\"剩餘強化任意門：" + chr.getChronosphere() + "。");
                chr.changeMap(moveTo, moveTo.getPortal(0));
        ...
        c.announce(CWvsContext.showChronosphere(chr));
    }
}
