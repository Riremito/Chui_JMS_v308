...
//        c.announce(ServerConstants.MAPLE_TYPE == ServerConstants.MapleType.GLOBAL ? CField.spawnFlags(null) : LoginPacket.changeBackground(backgrounds));
        for (WorldConstants.Option servers : WorldConstants.values()) {
            if (servers.show()) {
                c.announce(LoginPacket.getServerList(servers));
...
/*        if (!hasCharacters) {
    ...
    public static void CharlistRequest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        System.out.println("用戶端地址: " + c.getSession().remoteAddress().toString().split(":")[0] + " 連接到伺服器: " + server + " 頻道: " + channel + "");
...
//            c.announce(LoginPacket.getJobOpen());
            c.announce(LoginPacket.getCharList(c.getSecondPassword(), c.getCharacterPos(), chars, c.getCharacterSlots(), server == 45));
        } else {
            c.getSession().close();
        }
    }
    ...
    public static void changeCharPosition(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (c.getCharacterPos().contains(pos)) {
                ...
                System.out.println("角色位置更變出錯: 非本帳號的角色ID");
                return;
            }
        }
...
/*        if (li.isForbiddenName(name) && !c.isGM()) {
    ...
    public static void CreateChar2Pw(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        // 驗證碼
//        c.announce(LoginPacket.createCharCheckCode(CheckCodeImageCreator.createCheckCode().getRight(), (byte) 0, (byte) 1, (byte) 1, (byte) 0));
    }
    
    public static void CreateCharClick(final LittleEndianAccessor slea, final MapleClient c) {
        c.announce(LoginPacket.secondPasswordWindows());
    }
    ...
    public static void CreateChar(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        int keymapType = 1/*slea.readInt()*/; //按鍵模式: 0-基本模式; 1-進階模式
...
/*        if ((subcategory != 0 && job != JobType.影武者) || (subcategory != 1 && job == JobType.影武者) 
        ...
        int[][] skills = new int[][]{
            ...
            {60001216/*洗牌交換 防禦模式*/, 60001217/*洗牌交換 攻擊模式*/, 60001218 /*縱向連接*/, 60000219/*變身*/, 60000222/*鋼鐵意志*/},//凱撒Kaiser
            {60011216, 60010217, 60011218, 60011219, 60011220, 60011221, 60011222},//天使破壞者AngelicBuster
            {},//重炮指揮官Cannoneer
            {/*30021238刀舞*/},//傑諾Xenon
    ...
    public static void Character_WithoutSecondPassword(final LittleEndianAccessor slea, final MapleClient c, final boolean haspic, final boolean view) {
        ...
        c.announce(CField.getServerIP(c, Integer.parseInt(ChannelServer.getInstance(c.getChannel()).getIP().split(":")[1]), charId));
    }
