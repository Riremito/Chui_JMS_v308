...
//            c.getSession().close();
    ...
    public static void EnterCS(final CharacterTransfer transfer, final MapleClient c) {
        ...
        c.announce(CSPacket.warpCS(c));
        c.announce(CSPacket.warpCSInfo(c));
//        c.announce(CSPacket.disableCS());
        // 0x159
//        c.announce(CSPacket.CashUse(10500002, 0x32, 0xD2, 20130320, 20130326));
//        c.announce(CSPacket.CashUse2(0x3));
        c.announce(CSPacket.getCSInventory(c));
//        c.announce(CSPacket.CashUse3());
        //c.announce(CSPacket.CS_Picture_Item()); // Updated - Need to check if nothing changed
        //c.announce(CSPacket.CS_Top_Items()); // Updated to v146.1
        //c.announce(CSPacket.CS_Special_Item()); // Updated to v146.1
        //c.announce(CSPacket.CS_Featured_Item()); // Updated to v146.
//        c.announce(CSPacket.doCSMagic());
//        c.announce(CSPacket.getCSGifts(c));
//        c.announce(CSPacket.showCSAccount(c));
//        c.announce(CSPacket.sendWishList(c.getPlayer(), false));
//        c.announce(CSPacket.CashUse4());
        c.announce(CSPacket.showNXMapleTokens(c.getPlayer()));
//        c.announce(CSPacket.showNXMapleTokens(c.getPlayer()));
    }
            ...
            if (!CouponCodeAttempt(c)) {
                ...
                switch (type) {
                    ...
                    case 3:
                        ...
                        if (itez == null) {
                            c.announce(CSPacket.sendCSFail(0));
                            return;
                        }
                ...
                c.announce(CSPacket.showCouponRedeemedItem(itemz, mesos, maplePoints, c));
                doCSPackets(c);
            }
        } else {
            if (CouponCodeAttempt(c) == true) {
                c.announce(CSPacket.sendCSFail(48)); //A1, 9F
            } else {
                c.announce(CSPacket.sendCSFail(info == null ? 14 : 17)); //A1, 9F
            }
        }
    }
...
//        System.out.println("action " + action);
        if (action == 0) { // 兌換券
            ...
            if (itemz != null) {
                ...
                c.announce(CSPacket.showBoughtCSItem(itemz, item.getSN(), c.getAccID()));
            } else {
                c.announce(CSPacket.sendCSFail(0));
            }
            ...
            if (item != null && chr.getCSPoints(toCharge) >= item.getPrice()) {
                ...
                if (itemz != null && itemz.getUniqueId() > 0 && itemz.getItemId() == item.getId() && itemz.getQuantity() == item.getCount()) {
                    chr.getCashInventory().addToInventory(itemz);
                    //c.announce(CSPacket.confirmToCSInventory(itemz, c.getAccID(), item.getSN()));
                    c.announce(CSPacket.showBoughtCSItem(itemz, item.getSN(), c.getAccID()));
                } else {
                    c.announce(CSPacket.sendCSFail(0));
                }
            ...
            c.announce(CSPacket.sendWishList(chr, true));
            ...
            if (coupon) {
                ...
                if ((type == MapleInventoryType.SETUP ? chr.getCSPoints(toCharge) >= 150 : chr.getCSPoints(toCharge) >= 180) && chr.getInventory(type).getSlotLimit() < 89) {
                    ...
                    c.announce(CSPacket.sendCSFail(0xA4));
                }
            } else {
            ...
            if (item != null && item.getQuantity() > 0 && MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                ...
                if (pos >= 0) {
                    ...
                    c.announce(CSPacket.confirmFromCSInventory(item_, pos));
                } else {
                    c.announce(CSPacket.sendCSFail(0xB1));
                }
            } else {
                c.announce(CSPacket.sendCSFail(0xB1));
            }
            ...
            if (item1 == null) {
                c.announce(CSPacket.showNXMapleTokens(chr));
                return;
            }
            if (cs.getItemsSize() < 100) {
                ...
                c.announce(CSPacket.confirmToCSInventory(item1, c.getAccID(), sn));
            } else {
            ...
            if (item == null || !ItemConstants.類型.特效戒指(item.getId()) || c.getPlayer().getCSPoints(toCharge) < item.getPrice() || msg.getBytes().length > 73 || msg.getBytes().length < 1) {
                ...
                c.announce(CSPacket.sendCSFail(0xA6));
                doCSPackets(c);
                return;
            } else if (c.getPlayer().getCashInventory().getItemsSize() >= 100) {
                c.announce(CSPacket.sendCSFail(0xB1));
            ...
            if (info == null || info.getLeft() <= 0 || info.getLeft() == c.getPlayer().getId()) {
                c.announce(CSPacket.sendCSFail(0xB4));
                doCSPackets(c);
                return;
            } else if (info.getMid() == c.getAccID()) {
                c.announce(CSPacket.sendCSFail(0xA3));
                doCSPackets(c);
                return;
            } else {
                if (info.getRight() == c.getPlayer().getGender() && action == 30) {
                    c.announce(CSPacket.sendCSFail(0xA1));
                    doCSPackets(c);
                    return;
                }
            ...
            chr.modifyCSPoints(toCharge, -item.getPrice(), false);
            c.announce(CSPacket.showBoughtCSPackage(ccz, c.getAccID()));
            ...
            if (item == null || !MapleItemInformationProvider.getInstance().isQuestItem(item.getId())) {
                ...
                c.announce(CSPacket.sendCSFail(0xB8));
                doCSPackets(c);
                return;
            ...
            c.announce(CSPacket.showBoughtCSQuestItem(item.getPrice(), (short) item.getCount(), pos, item.getId()));
        } else if (action == 49) {
//            c.announce(CSPacket.updatePurchaseRecord());
...
//            }
            ...
            //c.announce(CSPacket.sendRandomBox(uniqueid, new Item(1302000, (short) 1, (short) 1, (short) 0, 10), (short) 0));
...
//        }
        ...
        if (info == null || info.getLeft() <= 0 || info.getLeft() == c.getPlayer().getId() || info.getMid() == c.getAccID()) {
            c.announce(CSPacket.sendCSFail(0xA2));
            doCSPackets(c);
            return;
            ...
            c.announce(CSPacket.sendGift(item.getPrice(), item.getId(), item.getCount(), partnerName, true));
        }
        doCSPackets(c);
    }
...
//        System.out.println("Scategory " + Scategory);
        if (Scategory == 103) {
            ...
            c.announce(CSPacket.addFavorite(itemSn));
            ...
            c.announce(CSPacket.Like(item));
        } else if (Scategory == 109) {
            c.announce(CSPacket.Favorite(c.getPlayer()));
            ...
            if (category == 1060100) {
                c.announce(CSPacket.showNXChar(category));
                c.announce(CSPacket.changeCategory(category));
            } else {
//                System.err.println(category);
    ...
    public static void doCSPackets(MapleClient c) {
        ...
        //c.announce(CSPacket.doCSMagic());
        //c.announce(CSPacket.getCSGifts(c));
        //c.announce(CWvsContext.BuddylistPacket.updateBuddylist(c.getPlayer().getBuddylist().getBuddies()));
        c.announce(CSPacket.showNXMapleTokens(c.getPlayer()));
//        c.announce(CSPacket.sendWishList(c.getPlayer(), false));
//        c.announce(CSPacket.showNXMapleTokens(c.getPlayer()));
        c.announce(CSPacket.getCSInventory(c));
